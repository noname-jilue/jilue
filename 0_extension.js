game.import("extension",function(lib,game,ui,get,ai,_status){return {
	...
	content:function (config){
		...
		lib.group.push('肺炎');
		lib.translate.肺炎='<font color="#000000"><span style="font-size:20px">肺炎</font></span>';
		if(config.simple_name=='hide'){
			get.slimName=function(str){
				var str2=lib.translate[str];
				if(!str2) return '';
				if(str2.indexOf('SR旧')==0){
					str2=str2.slice(3);
				}
				...
				else if(str2.indexOf('UR')==0){
					str2=str2.slice(2);
				}
				else if(str2.indexOf('TW')==0){
					str2=str2.slice(2);
				}
				else if(str2.indexOf('兵谋')==0){
					str2=str2.slice(2);
				}
				else if(str2.indexOf('SUR')==0){
					str2=str2.slice(3);
				}
				else if(str2.indexOf('TM')==0){
					str2=str2.slice(2);
				}
				else if(str2.indexOf('☆SP')==0){
					str2=str2.slice(3);
		...
		if(config.jlsg_sc){
			for(var i in lib.characterPack['jlsg_sc']){	
		...
		if(config.jlsg_sr){
			for(var i in lib.characterPack['jlsg_sr']){	
				if(lib.character[i][4].indexOf("forbidai")<0){
					lib.character[i][4].push("forbidai");
				}
			}
		...
		if(config.jlsg_sk){
			for(var i in lib.characterPack['jlsg_sk']) {	
				if(lib.character[i][4].indexOf("forbidai")<0){
					lib.character[i][4].push("forbidai");
				}
			}
		...
		if(config.jlsg_soul){
			for(var i in lib.characterPack['jlsg_soul']) {	
				if(lib.character[i][4].indexOf("forbidai")<0){
					lib.character[i][4].push("forbidai");
				}
			}
...
},precontent:function(jilvesanguo){
       if(jilvesanguo.enable){
			game.import('character',function(){
                var jlsg_sc = {
                    name:'jlsg_sc',
                    connect:true,
					character:{
						jlsgsc_guanping:["male",'shu',4,["jlsg_longying"],[]],
						sc_madai:['male','shu',4,['mashu','sc_qianxi'],[]],
						sc_guanzhang:['male','shu',4,['sc_fuhun'],[]],
						jlsgsc_lvmeng:["male",'wu',4,["jlsg_keji"],[]],
						jlsgsc_new_luxun:["male",'wu',4,["jlsg_shaoying","jlsg_zonghuo"],[]],
						jlsgsc_bingyi:["male","肺炎",3,["jlsg_qinran","jlsg_qinran_kangxing"],[]],
						jlsgsc_relvmeng:["male","wu",4,["jlsg_lanjuan","jlsg_shibie"],[]],
						jlsgsc_zilong:["male","shu",3,["jlsg_longdan"],[]],
						jlsgsc_erluxun:["male","wu",4,["jlsg_ershenbing","jlsg_erqiying"],[]],
						jlsgsc_recaoying:["female","wei",3,["jlsg_re_xieji1","jlsg_re_jiyu"],[]],
					},
					characterIntro:{
						jlsgsc_guanping:'关平（196-220年）东汉末年名将关羽之子，关羽率军攻打樊城时，孙权吕蒙趁机背盟偷袭荆州，关羽后院起火只得从前线战场撤退。因为关羽的大本营江陵被东吴军攻占，关羽军中所有将士的全家老小都被东吴军控制，引发关羽军将士大溃散。关平与其父在撤退到麦城时中了东吴军埋伏一同被擒，最后一起被斩于临沮县。',
						sc_madai:'马岱（生卒年不详），扶风茂陵（今陕西兴平）人。三国时期蜀汉将领，马超的从弟。早年追随马超大战曹操，反攻陇上，围攻成都，汉中之战等。后在诸葛亮病逝后受杨仪派遣斩杀了蜀将魏延。曾率领军队出师北伐，被魏将牛金击败而退还。官至平北将军，陈仓侯',
						sc_guanzhang:'关兴（生卒年不详），字安国，河东解县（今山西运城）人，三国时期蜀汉官员，东汉末年名将关羽次子，关平之弟，弱冠时就官至侍中、中监军，随后英年早逝。《三国演义》中随刘备兴兵伐吴斩杀潘璋夺回父亲的青龙偃月刀，为蜀汉后期重要将领，并与张飞之子张苞结义，多次参加诸葛亮北伐，后在诸葛亮第六次北伐前去世。<br>张苞，蜀汉车骑将军张飞的长子，早夭 [1]  ，留有一子张遵。因此由次子张绍袭爵。在《三国演义》中他和关羽次子关兴是关系很好的结拜兄弟，两人并称"小关张"。',
					},
					characterTitle:{
					   jlsgsc_guanping:"作者：没有魂的背影",
					   jlsgsc_lvmeng:'作者：没有魂的背影',
					   jlsgsc_new_luxun:"作者：没有吊的灵魂",
					   jlsgsc_bingyi:"作者：武汉人民",
					   sc_guanzhang:'作者：<font color=Purple>仲哥</font>',
					   sc_madai:'作者：<font color=Red>仲哥</font>',
					},
					skill:{
						jlsg_qinran_kangxing:{
							forced:true,
						...
						jlsg_qinran:{
							...
							filter:function(){
								for(var i=0;i<game.players.length;i++){
									if(!game.players[i].storage.jlsg_qinran) return true;
								}
								return false;
							},
							filterTarget:function(card,player,target){
								return !target.storage.jlsg_qinran&&player!=target;
							},
							content:function(){
								target.addTempSkill('jlsg_qinran_qin',{player:'phaseDrawBefore'});
								target.storage.jlsg_qinran=true;
								target.storage.jlsg_qinran2=player;
							},
							ai:{
								order:4,
								result:{
							...
							subSkill:{
								qin:{
									...
									content:function(){
										'step 0'
										player.storage.jlsg_qinran=false;
										player.loseHp();
										'step 1'
										delete player.storage.jlsg_qinran2;
									},
									intro:{
										content:"该角色回合开始时，将失去1点体力",
									},					
								},
							}
						},
						jlsg_shaoying:{
							...
							content:function(){
								"step 0"
								player.chooseTarget(get.prompt('jlsg_shaoying'),function(card,player,target){
									return get.distance(trigger.player,target)<=1&&target!=player&&trigger.player!=target;
								}).ai=function(target){
								...
								if(result.bool){
									...
									player.logSkill('jlsg_shaoying',trigger.player);
								...
								"step 2"
								if(event.bool){
									event.target.damage('fire',trigger.player);
								}
							}
						},
						jlsg_zonghuo:{
							...
							content:function(){
								"step 0"
								player.chooseToDiscard(get.prompt('jlsg_zonghuo'),'he').ai=function(card){
								...
								"step 1"
								if(result.bool){
									player.logSkill('jlsg_zonghuo',player,'fire');
									trigger.nature='fire';
								}
							}
						},
						jlsg_shibie:{
							...
							unique:true,
							juexingji:true,
							derivation:"jlsg_cexin",
							trigger:{
								player:"phaseZhunbeiBegin",
							},
							forced:true,
							filter:function (event,player){
								if(player.storage.jlsg_shibie) return false;
								return player.storage.jlsg_lanjuan && player.storage.jlsg_lanjuan.length>=3;
							},
							content:function (){
								player.storage.jlsg_shibie=true;
								player.recover(2);
								player.addSkill('jlsg_cexin');
								player.awakenSkill('jlsg_shibie');
							},
						},
						jlsg_keji:{
							mod:{
							...
							content:function (){
								player.draw(2);
								player.addSkill('jlsg_keji_phaseBefore');
							},
						},
						jlsg_keji_phaseBefore:{
							audio:true,
							trigger:{
								player:"phaseBefore",
							},
							...
							content:function (){
								'step 0'
								player.chooseToDiscard('he','克己','弃置2张牌，或失去1点体力',2).ai=function(card){
									return 8-get.value(card);
								};
								'step 1'
								if(result.bool){
									player.removeSkill('jlsg_keji_phaseBefore');
								}else{
									player.loseHp();
									player.removeSkill('jlsg_keji_phaseBefore');
								}
						...
						jlsg_lanjuan:{
							...
							group:"jlsg_Zhu_buff",
							filter:function (event, player) {
								return player.storage.jlsg_lanjuan.length<4;
							},
							content:function (){
								"step 0"
								trigger.cancel();
								...
								event.time=get.utc();
								game.addVideo('showCards',player,['览卷',get.cardsInfo(event.cards)]);
								...
								var next=player.chooseButton(suits.length,true);
								...
								if(result.bool&&result.links){
									...
									if (!player.storage.jlsg_lanjuan) {
										player.storage.jlsg_lanjuan = [];
									}
									player.storage.jlsg_lanjuan = player.storage.jlsg_lanjuan.concat(cards);
										if(player.storage.jlsg_lanjuan.length>4){
									player.chooseCardButton("［卷］已超过4张请弃置"+(player.storage.jlsg_lanjuan.length-4)+"张［卷］", (player.storage.jlsg_lanjuan.length-4),player.storage.jlsg_lanjuan,true);
							
									 }
									player.markSkill('jlsg_lanjuan');
								...
								"step 3"
								if (result.links) {
									player.storage.jlsg_lanjuan.remove(result.links);
									game.cardsDiscard(result.links);          
									player.syncStorage('jlsg_lanjuan');
								}
								"step 4" 
						...
						jlsg_cexin:{
							...
							mod:{
								maxHandcard:function (player,num){
									return num+player.storage.jlsg_lanjuan.length;
								},
							},
							filter:function (event, player) {
								if (!player.storage.jlsg_lanjuan) return false;
								if (_status.currentPhase != player) return false;
								//if (player.storage.jlsg_lanjuan.length>=1) return true;
							...
							content:function () {
								...
								if(goon){
									player.chooseTarget('测心：是否额外指定1名' + get.translation(trigger.card) + '的目标？', function (card, player, target) {
								...
								'step 1'
								if (result.bool) {
									player.logSkill('jlsg_cexin',result.targets[0]);
									if (!event.isMine()) game.delayx();
									event.target = result.targets[0];
									player.chooseCardButton('选择弃置1张［卷］',true, player.storage.jlsg_lanjuan).ai = function (button) {
										return true;
									};
								}
								else {
								...
								'step 2'
								if (event.target && result.links) {
									player.logSkill('jlsg_cexin', event.target);
									player.storage.jlsg_lanjuan.remove(result.links[0]);
									player.markSkill('jlsg_lanjuan');
									player.syncStorage('jlsg_lanjuan');
									if (!player.storage.jlsg_lanjuan.length) player.unmarkSkill('jlsg_lanjuan');
									trigger.targets.add(event.target);
								}
								event.finish();
								'step 3'
								player.chooseTarget('测心：是否减少1名' + get.translation(trigger.card) + '的目标？', function (card, player, target) {
								...
								'step 5'
								if (result.links) {
									player.logSkill('jlsg_cexin', event.target1);
									player.storage.jlsg_lanjuan.remove(result.links[0]);
									player.markSkill('jlsg_lanjuan');
									player.syncStorage('jlsg_lanjuan');
									if (!player.storage.jlsg_lanjuan.length) player.unmarkSkill('jlsg_lanjuan');
									...
									for (var i = 0; i < event.targets1.length; i++) {
									  trigger.targets.remove(event.targets1[i]);
									}
									if(trigger.card.name=="wugu")trigger.stocktargets=trigger.targets;
										trigger.untrigger();
						...
						jlsg_ershenbing:{
							...
							marktext:'营',
							init:function (player){
								player.storage.jlsg_ershenbing=[];
							},
							filter:function(event,player){
								return player.storage.jlsg_ershenbing.length < 2 && event.num>0;
							},
							intro:{
								content:"characters",
							},
							content:function (){
								...
								for(var i in lib.character){
									...
									if(player.storage.jlsg_ershenbing.contains(i)) continue;
									if(list2.contains(i)) continue;
									list.push(i);
								}
								var name = list.randomGet();
								player.storage.jlsg_ershenbing.push(name);
								player.markSkill('jlsg_ershenbing');
								player.syncStorage("jlsg_ershenbing");
								var skills=lib.character[name][3];
								for (var i = 0; i < skills.length; i++) {
									var info = lib.skill[skills[i]];
									if (info.charlotte || (info.unique && !info.gainable) || info.juexingji || info.limited || info.zhuSkill) continue;
									player.addSkill(skills[i]);
								}
								event.dialog=ui.create.dialog('<div class="text center">'+get.translation(player)+'发动了〈神兵〉',[[name],'character']);
								game.delay(2);
								if (trigger.num > 1 && player.storage.jlsg_ershenbing.length < 2) event.redo();
								'step 2'
								event.dialog.close();
							},
							ai:{
						...
						jlsg_erqiying: {
							...
							filter: function (event, player) {
								return player.storage.jlsg_ershenbing && player.storage.jlsg_ershenbing.length && event.num > 0;
							},
							forced:true,
							content:function (){
								"step 0"
								if (player.storage.jlsg_ershenbing && player.storage.jlsg_ershenbing.length) {
									player.chooseButton(['选择你需要弃置的［营］', [player.storage.jlsg_ershenbing, 'character']], true);
								}
								"step 1"
								if (result.bool) {
									var name = result.links;
									player.storage.jlsg_ershenbing.remove(name);
									player.markSkill('jlsg_ershenbing');
									player.syncStorage("jlsg_ershenbing");
									var skills = lib.character[name][3];
									for (var i = 0; i < skills.length; i++) {
								...
								"step 2"
								if (player.storage.jlsg_ershenbing && player.storage.jlsg_ershenbing.length && trigger.num > 1) {
									event.goto(0);
								}
							},
						},
						jlsg_re_jiyu: {
							...
							filter: function (event, player) {
								return player.storage.jlsg_re_xieji && player.storage.jlsg_re_xieji.length;
							},
							group:['jlsg_re_jiyu2'],
							direct:true,
							content: function () {
								'step 0'
								player.chooseTarget(get.prompt('jlsg_re_jiyu'), "移去1枚［计］，然后令1名其他角色获得1枚［计］", function (card, player, target) {
									return target != player;
								}).ai = function (target) {
									var player = _status.event.player;
									if (player.storage.jlsg_re_xieji.length > 1) return get.attitude(player, target);
									return -1;
								};
								'step 1'
								if (result.targets) {
									event.target = result.targets[0];
										player.chooseCardButton(player.storage.jlsg_re_xieji, '选择1枚［计］',true).set('ai', function (button) {
											get.value(button.link);
										});
								}
								'step 2'
								if (result.links) {
									player.logSkill('jlsg_re_jiyu', target);
									target.addSkill('jlsg_re_xieji');
									target.addSkill('jlsg_re_jiyu2');
									player.storage.jlsg_re_xieji.remove(result.links[0]);
									player.markSkill('jlsg_re_xieji');
									player.syncStorage("jlsg_re_xieji");
									player.$give(result.links[0], target);
									target.storage.jlsg_re_xieji = target.storage.jlsg_re_xieji.concat(result.links[0]);
									target.markSkill('jlsg_re_xieji');
									target.syncStorage("jlsg_re_xieji");
									if (!player.storage.jlsg_re_xieji.length) player.unmarkSkill('jlsg_re_xieji');
								}
							},
						},
						jlsg_re_jiyu2: {
							...
							direct:true,
							filter: function (event, player) {
								return player.storage.jlsg_re_xieji&&player.storage.jlsg_re_xieji.length&&player.countCards('h')==0;
							},
							content: function () {
								'step 0'
								player.chooseCardButton(player.storage.jlsg_re_xieji, '选择弃置1枚［计］，防止此伤害。').set('ai', function (button) {
									//get.value(button.link);
									return true;
								});
								'step 1'
								if (result.links) {
									player.logSkill('jlsg_re_jiyu2');
									var card = result.links[0];
									player.storage.jlsg_re_xieji.remove(card);
									...
									player.syncStorage("jlsg_re_xieji");
									trigger.cancel();
									if(player.storage.jlsg_re_xieji.length==0){
										player.removeSkill('jlsg_re_xieji');
										player.removeSkill('jlsg_re_jiyu2');
										player.unmarkSkill('jlsg_re_xieji');
									}
								}
							},
						},
						jlsg_re_xieji1: {
							...
							group:['jlsg_re_xieji',"jlsg_re_xieji3"],
						
							filter: function (event, player) {
								if (player.storage.jlsg_re_xieji && player.storage.jlsg_re_xieji.length) return false;
								return true;
							},
							content: function () {
								trigger.num++;
								player.addSkill('jlsg_re_xieji2');
							},
						},
						jlsg_re_xieji2: {
							trigger: {
							...
							filter: function (event, player) {
								return player.storage.jlsg_re_xieji && player.storage.jlsg_re_xieji.length == 0;
							},
							direct: true,
							unique:true,
							content: function () {
								'step 0'
								player.chooseCard(get.prompt('jlsg_re_xieji2'),'h','你可以将至多' + (4 - player.storage.jlsg_re_xieji.length) + '张手牌置于武将牌，称为［计］最多4枚',[1,4 - player.storage.jlsg_re_xieji.length]).set('ai',function (card) {
									var player = _status.event.player;
								...
								if (result.cards) {
									...
									player.storage.jlsg_re_xieji = player.storage.jlsg_re_xieji.concat(result.cards);
									player.markSkill('jlsg_re_xieji');
									player.syncStorage("jlsg_re_xieji");
									if (!player.storage.jlsg_re_xieji.length) player.unmarkSkill('jlsg_re_xieji');
								}
						...
						jlsg_re_xieji3: {
							...
							forced:true,
							content: function () {
							player.removeSkill('jlsg_re_xieji2');
							},
						},
						sc_fuhun:{
							audio:'ext:民间极略:2',
							...
							content:function(){
								...
								trigger.finish();
								trigger.untrigger();
								var num=trigger.num+Math.min(player.getDamagedHp(),2);
								event.cards=get.cards(num);
								player.showCards(event.cards);
								'step 1'
								if(player.isDamaged()||(get.color(event.cards[0])!=get.color(event.cards[1]))){
									player.addTempSkill('sc_wusheng');
									player.addTempSkill('sc_paoxiao');
								}
								...
								effect:{
									target:function(card,player,target){
										if(get.tag(card,'damage')||get.tag(card,'loseHp')){
											if(target.isHealthy())return [1,0.9];
										}
									}
								}
							}
						...
						sc_qianxi:{
							audio:['qianxi','oldqianxi'].randomGet(),
							group:['sc_qianxi_a','sc_qianxi_b'],
							subSkill:{
								a:{
									audio:"qianxi",
									trigger:{player:'phaseZhunbeiBegin'},
									description:'准备阶段，你可以摸一张牌，并弃置一张牌（手牌数不大于体力值时可收回此牌），然后令一名距离为1的角色不能使用或打出与你弃置的牌颜色相同的手牌直到回合结束。',
									content:function(){
										"step 0"
										player.draw();
										...
										player.chooseCardButton(result.cards,'潜袭：是否收回'+get.translation(result.cards[0])+'？').set('ai',function(button){
										...
										player.chooseTarget(function(card,player,target){
											...
											var distance=0;
											if(get.distance(player,target,'attack')<=1)distance=1;
											if(color=='red'&&(target.hasCard('tao','h')||target.hasShan()))return 6+distance;
											return Math.max(5-target.hp,1)+Math.max(4-target.countCards('h'),1)/2+distance;
										});
										"step 4"
										if(result.bool&&result.targets.length){
											result.targets[0].storage.sc_qianxi_c=event.color;
											result.targets[0].addTempSkill('sc_qianxi_c');
											player.line(result.targets,'green');
											game.addVideo('storage',result.targets[0],['sc_qianxi_c',event.color]);
										}
									},
								},
								b:{
									...
									check:function(event,player){
										...
										if(event.player.hp==event.player.maxHp-1&&event.player.maxHp<=4) return att<=0;
										if(event.player.hasSkillTag('maixie'))return att<=0;
										return att>0;
									},
									description:'当你使用【杀】对距离为1的目标角色造成伤害时，你可以进行一次判定，若判定结果不为红桃，你防止此伤害，令其减1点体力上限',
									filter:function(event,player){
								...
								c:{
									...
									onremove:function(player){
										player.unmarkSkill('sc_qianxi_c');
										delete player.storage.sc_qianxi_c;
									},
									mod:{
										cardEnabled:function(card,player){
											if(get.color(card)==player.storage.sc_qianxi_c) return false;
										},
									...
									ai:{
										effect:{
											target:function(card,player,target){
												if(!target.storage.sc_qianxi_c)return false;
												var color=target.storage.sc_qianxi_c;
												if(color=='red'){
													if(card.name=='sha'){
														return [1,-1.5];
													}
												}else{
													if(['juedou','nanman'].contains(card.name))return [1,-1.5];
												}
											},
										}
									}
						...
						jlsg_longying:{
							...
							content:function (){
								...
								var next=player.chooseToDiscard(1,'he','龙吟：是否弃置1张牌？<br><li>若弃置的牌为<span style="color:#FF0000">红色</span>则'+translation+'的'+translation2+'不计入出杀次数。<br><li>若弃置的牌为<span style="color:#FF00FF">黑色</span>则'+translation+'的'+translation2+'可以额外指定1个目标。',function(card,player){
									return true;
								});
								...
								"step 1"
								if(result.bool){
									player.logSkill('jlsg_longying',trigger.player);
									if(get.color(result.cards[0])=='red'){
										...
										event.usesha=true;
										game.log(trigger.player,'#y使用的',trigger.card,'可以额外指定1个目标');
									}
								}
								else{
									event.finish();        
								...
								if(event.usesha==true){
									trigger.player.chooseTarget('龙吟：是否为'+get.translation(trigger.card)+'额外指定1个目标？',function(card,player,target){
					...
					translate:{
						...
						sc_guanzhang:'关兴张苞',
						sc_wusheng:'武圣',
						sc_wusheng_info:'你可以将一张红色牌当做【杀】使用或打出。你使用的方片杀没有距离限制。',
						sc_paoxiao:'咆哮',
						sc_paoxiao_info:'锁定技，出牌阶段，你使用【杀】没有数量限制。若你于此出牌阶段内使用过【杀】，则你本回合内使用【杀】没有距离限制。',
						sc_qianxi:'潜袭',
						sc_qianxi_info:'<li>准备阶段，你可以摸一张牌，并弃置一张牌（手牌数不大于体力值时可收回此牌），然后令一名距离为1的角色不能使用或打出与你弃置的牌颜色相同的手牌直到回合结束。<li>当你使用【杀】对距离为1的目标角色造成伤害时，你可以进行一次判定，若判定结果不为红桃，你防止此伤害，令其减1点体力上限',
						sc_fuhun:'父魂',
						sc_fuhun_info:'摸牌阶段开始时，你可以放弃摸牌，改为从牌堆顶亮出X张牌并获得之，若亮出的牌颜色不同，你获得技能“武圣”、“咆哮”（若你已受伤则跳过此条件），直到回合结束（X为你应当摸取的牌数+你已损失体力值[最大为2]）。',
						jlsgsc_guanping:"关平",
						jlsg_longying:"龙吟",
						jlsg_longying_info:"每当1名角色在其出牌阶段使用【杀】时，你可弃置1张牌来触发以下效果：1.若弃置的牌为红色，不计入出牌阶段使用次数，然后你摸1张牌；2.若弃置的牌为黑色，可以额外指定1个目标。",
						jlsgsc_shenliubei:"新神刘备",
						jlsg_sc_longnu:"龙怒",
						jlsg_sc_longnu_red:'龙怒·火杀',
						jlsg_sc_longnu_black:'龙怒·雷杀',
						jlsg_sc_longnu_info:"锁定技，回合开始时你摸X张牌（X为你的体力上限减1半向上取整）进行1次判定：黑色，本回合你的黑色手牌视为雷【杀】且不计出牌次数并无视防具，红色的【杀】不可使用；红色，本回合你的红色手牌视为火【杀】且不计出牌次数并无视距离，黑色的【杀】不可使用。",
						jlsgsc_re_zhouyu:"周瑜",
						jlsg_new_yingzi:"英姿",
						jlsg_new_yingzi_info:"锁定技，你的回合内，使用1张牌并摸1张牌。",
						jlsgsc_recaoying:"曹婴",
						jlsg_re_xieji:"蝎计·判定",
						jlsg_re_xieji1:"蝎计",
						jlsg_re_xieji2:"蝎计·屯牌",
						jlsg_re_xieji1_info:"摸牌阶段开始时，若你没有［计］时，你可以多摸1张牌；若如此做，出牌阶段结束时，你可以将至少1张手牌，置入你的武将牌，称为［计］最多4枚；下回合出牌阶段开始时，你可以进行判定，然后可以将1枚与判定牌相同花色的［计］弃置并摸2张牌，然后你继续判定，直到你无法弃置与判定牌相同花色的［计］为止。",
						jlsg_re_xieji_info:"你可以进行判定，然后可以将1枚与判定牌相同花色的［计］弃置并摸2张牌，然后你继续判定，直到你无法弃置与判定牌相同花色的［计］为止。",
						jlsg_re_jiyu:"计育",
						jlsg_re_jiyu2:"计育·免伤",
						jlsg_re_jiyu_info:"你的回合结束时，若你有标记时，你可以将1枚［计］移至1名其他角色，该角色出牌阶段可执行［蝎计］的判定；计：当你受到1次伤害时，若没有手牌时，可以弃置1枚［计］来抵消此伤害。",
						jlsgsc_erluxun:"新界陆逊",
						jlsg_ershenbing:"神兵",
						jlsg_ershenbing_info:"变化技，当你受到1点伤害后，你可以随机获得1张未加入游戏的吴国武将牌置入你的武将牌上（觉醒技、主公技、限定技和变化技除外），称为［营］最多2枚；你拥有［营］的所有技能。",
						jlsg_erqiying:"弃营",
						jlsg_erqiying_info:"锁定技，每当你回复1点体力后，你须弃置1枚［营］。",
						jlsgsc_zilong:"子龙",
						jlsg_longdan:"龙胆",
						jlsg_longdan1:"龙胆·杀",
						jlsg_longdan2:"龙胆·闪",
						jlsg_longdan3:"龙胆·桃",
						jlsg_longdan4:"龙胆·酒",
						jlsg_longdan_info:"你可以将1张手牌当做【杀】、【闪】、【桃】、【酒】使用或打出。",
						jlsg_lanjuan:"览卷",
						jlsg_shibie:"士别",
						jlsg_cexin:"测心",
						jlsgsc_relvmeng:"新界吕蒙",
						jlsg_lanjuan_info:"若你的武将牌顶的标记不大于4时，你可以放弃摸牌，改为从牌堆顶亮出5张牌，然后选择获得不同花色的牌各1张；其余的牌置入你的武将牌顶，称为［卷］最多4枚。锁定技，若你的身份为“主公”时，你不增加体力上限。",
						jlsg_shibie_info:"觉醒技，锁定技，你的回合开始时，若你的［卷］为3或更多，你须恢复2点体力，然后获得技能〈测心〉。测心：你的回合使用非延迟锦囊牌时，你可以弃置1枚［卷］，使该牌增加/减少1个目标；锁定技，你的手牌上限额外增加X张（X为标记数）。",
						jlsg_cexin_info:"你的回合使用非延迟锦囊牌时，你可以弃置1枚［卷］，使该牌增加/减少1个目标；锁定技，你的手牌上限额外增加X张（X为标记数）。",
						jlsgsc_new_luxun:"新陆逊",
						jlsg_zonghuo:"纵火",
						jlsg_shaoying:"烧营",
						jlsg_zonghuo_info:'你可以弃置1张牌将你即将造成的伤害转为火焰伤害。',
						jlsg_shaoying_info:'当你造成1次火焰伤害后，可指定距离受到伤害的该角色1以内的另1名角色，并展示牌堆顶的1张牌，若此牌为红色，该角色受到（之前受到你的火焰伤害的角色）1点火焰伤害。',
						jlsgsc_lvmeng:"吕蒙",
						jlsg_keji:"克己",
						jlsg_keji_phaseBefore:"克己·弃牌",
						jlsg_keji_info:"你的回合结束时，你将摸2张牌；你的回合开始时，若你的手牌大于你的体力上限时，你须弃置2张牌，若不此做，则失去1点体力；你的手牌上限额外X张牌（X为你的体力上限）。",
						jlsgsc_bingyi:'病疫',
						jlsg_qinran:'侵染',
						jlsg_qinran_kangxing:'抗性',
						jlsg_qinran_kangxing_info:"锁定技，你的进攻距离和防御距离+1；你的判定牌均为不生效；若你的身份为“主公”时，你不增加体力上限。",
						jlsg_qinran_info:"出牌阶段限1次，你可以指定1名其他角色，令其于其下回合开始时失去1点体力。",
					},					
				};			
                if(lib.device||lib.node){
                    for(var i in jlsg_sc.character){jlsg_sc.character[i][4].push('ext:民间极略/'+i+'.jpg');}
                }else{
                    for(var i in jlsg_sc.character){jlsg_sc.character[i][4].push('db:extension-民间极略:'+i+'.jpg');}
                }
                return jlsg_sc;
            });	
            lib.config.all.characters.push('jlsg_sc');
		    if(!lib.config.characters.contains('jlsg_sc')) lib.config.characters.remove('jlsg_sc');
            lib.translate['jlsg_sc_character_config'] = 'SC武将';   
            ...
            game.import('character',function(){
                var jlsg_sk = {
					...
					character:{
						...
						jlsgsk_dengzhi:['male','shu',3,['jlsg_hemeng','jlsg_sujian'],['die_audio']],
						jlsgsk_xuyou:['male','wei',3,['jlsg_yexi','jlsg_kuangyan'],['die_audio']],
						jlsgsk_zhangbu:['male','wu',3,['jlsg_chaochen','jlsg_quanzheng'],['die_audio']],
						jlsgsk_miheng:['male','qun',3,['jlsg_shejian','jlsg_kuangao'],['die_audio']],
						jlsgsk_zumao:['male','wu',4,['jlsg_yinbing'],['die_audio']],
						jlsgsk_huaxiong:['male','qun',5,['jlsg_fenwei','jlsg_shiyong'],['die_audio']],
						jlsgsk_sunce:['male','wu',4,['jlsg_angyang','jlsg_weifeng'],['zhu','die_audio']],
						jlsgsk_caoren:['male','wei',4,['jlsg_jushou'],['die_audio']],
						jlsgsk_gongsunzan:['male','qun',4,['jlsg_yicong','jlsg_muma'],['die_audio']],
						jlsgsk_sunqian:['male','shu',3,['jlsg_suiji','jlsg_fengyi'],['die_audio']],
						jlsgsk_maliang:['male','shu',3,['jlsg_yalv','jlsg_xiemu'],['die_audio']],
						jlsgsk_buzhi:['male','wu',3,['jlsg_zhejie','jlsg_fengya'],['die_audio']],
						jlsgsk_wangping:['male','shu',4,['jlsg_yijian','jlsg_feijun'],['die_audio']],
						jlsgsk_huangyueying:['female','shu',3,['jlsg_muniu','jlsg_liuma'],['die_audio']],
						jlsgsk_dongzhuo:['male','qun',6,['jlsg_baozheng','jlsg_lingnu'],['die_audio']],
						jlsgsk_chendao:['male','shu',4,['jlsg_zhongyong'],['die_audio']],
						jlsgsk_dingfeng:['male','wu',4,['jlsg_bozhan','jlsg_qingxi'],['die_audio']],
						
						jlsgsk_zhuran:['male','wu',4,['jlsg_danshou','jlsg_yonglie'],['die_audio']],
						jlsgsk_lukang:['male','wu',4,['jlsg_hengshi','jlsg_zhijiao'],['die_audio']],
						jlsgsk_lvlingqi:['female','qun',4,['jlsg_jiwux'],['die_audio']],
						jlsgsk_zhoucang:['male','shu',4,['jlsg_daoshi'],['die_audio']],
						jlsgsk_kongrong:['male','qun',3,['jlsg_lirang','jlsg_xianshi'],['die_audio']],
						jlsgsk_caochong:['male','wei',3,['jlsg_chengxiang','jlsg_renxin'],['die_audio']],
						jlsgsk_zhanglu:['male','qun',3,['jlsg_midao','jlsg_yishe','jlsg_pudu'],['die_audio']],
						jlsgsk_guanlu:['male','wei',3,['jlsg_zongqing','jlsg_bugua'],['die_audio']],
						jlsgsk_simazhao:['male','wei',3,['jlsg_zhaoxin','jlsg_zhihe'],['die_audio']],
						jlsgsk_yangxiu:['male','wei',3,['jlsg_caijie','jlsg_jilei'],['die_audio']],
						jlsgsk_liyan:['male','shu',4,['jlsg_yanliang'],['die_audio']],
						jlsgsk_jiping:['male','qun',3,['jlsg_duzhi','jlsg_lieyi'],['die_audio']],
						jlsgsk_sunhao:['male','wu',4,['jlsg_baoli'],['die_audio']],
						jlsgsk_zhugejin:['male','wu',3,['jlsg_huanbing','jlsg_hongyuan'],['die_audio']],
						jlsgsk_zhangxiu:['male','qun',4,['jlsg_huaqiang','jlsg_chaohuang'],['die_audio']],
						jlsgsk_sunluyu:['female','wu',3,['jlsg_huilian','jlsg_wenliang'],['die_audio']],
						jlsgsk_luzhi:['male','qun',3,['jlsg_jinglun','jlsg_ruzong'],['die_audio']],	
						jlsgsk_yuji:['male','qun',3,['jlsg_guhuo','jlsg_fulu'],['die_audio']],
						jlsgsk_zhangning:['female','qun',3,['jlsg_leiji','jlsg_shanxi'],['die_audio']],
						jlsgsk_guonvwang:['female','wei',3,['jlsg_gongshen','jlsg_jianyue'],['die_audio']],
						jlsgsk_chengyu:['male','wei',3,['jlsg_pengri','jlsg_danmou'],['die_audio']],
						jlsgsk_zhangren:['male','qun',4,['jlsg_fushe'],['die_audio']],
						jlsgsk_mizhu:['male','shu',3,['jlsg_ziguo','jlsg_shangdao'],['die_audio']],
						jlsgsk_zangba:['male','wei',4,['jlsg_hengjiang'],['die_audio']],
						jlsgsk_hejin:['male','qun',4,['jlsg_zhuanshan'],['die_audio']],
						jlsgsk_wangyi:['female','wei',3,['jlsg_zhenlie','jlsg_miji'],['die_audio']],
						jlsgsk_zuoci:['male','qun',3,['jlsg_qianhuan'],['zhu','die_audio']],
						
						jlsgsk_guanyu:['male','wei',4,['jlsg_wusheng','jlsg_danqi'],['die_audio']],
						jlsgsk_zhangbao:['male','qun',3,['jlsg_zhoufu','jlsg_yingbing'],['die_audio']],
					...
					skill:{
						jlsg_zhengyi:{
							audio:"ext:民间极略:2",
							group:['jlsg_zhengyi_sha','jlsg_zhengyi_shan','jlsg_zhengyi_tao','jlsg_zhengyi_phaseUse'],
							ai:{
								respondSha:true,
								respondShan:true,
								save:true,
								...
								sha:{
									...
									onuse:function(result,player){
										game.broadcastAll(function(){
											game.playAudio('..','extension','民间极略','jilsg_zhengyi'+Math.random()>0.5?1:2);
										});
										player.draw('nodelay');
									},
								...
								phaseUse:{
									...
									chooseButton:{
										...
										filter:function(button,player){
											...
											if(lib.card[name]){
												if(lib.card[name].enable){
													var enable=lib.card[name].enable;
													if(typeof enable=='function'){
														if(!enable({name:name},player))return false;
													}else{
						...
						jlsg_huage:{
							...
							content:function(){
								...
								if(target.countCards('he')){
									target.chooseToDiscard("化戈：请弃置至少1张牌，弃置的牌中每有【杀】，你便摸1张牌",'he',[1,Infinity],true).ai = function(card){
			...
			jlsg_bibu:{
				...
				filter:function(event,player){
					if(event.player==player)return false;
					if(player.countCards('h')<=player.hp&&!player.hasSkill('jlsg_bibu2')){
						player.draw();
						player.logSkill('jlsg_bibu1');
			...
			jlsg_duanlan:{
				...
				content:function(){
					...
					var targets=game.filterPlayer(function(current){return current!=player}).sort(lib.sort.seat);
					...
					player.chooseButton(true,[1,3]).set('createDialog',info).set('filterButton',function(button){
						...
						var maxNumCards=player.getCards('he',function(card){
							return get.value(card)<9&&!player.hasCard(function(card2){return card2.number>card.number})
						});
						var maxNum=maxNumCards.length?maxNumCards[0].number:0;
						var dngr=player.hp==1&&!player.hasCard(function(card){return card.name=='tao'||card.name=='jiu'});
					...
					'step 2'
					player.chooseToDiscard('断缆</br></br><div class="center text">弃置1张点数大于'+num+'的牌，或失去1点体力</div>',function(card){
				...
				ai:{
					...
					result:{
						player:function(player){
							...
							if(player.hp>2||player.hasCard(function(card){return card.number>10},'h')) return game.hasPlayer(function(current){
							...
							if(js.length){
								if(player.hasCard(function(card){return card.number>minNum1&&get.value(card)<9},'he')) return 1;
								if(!dngr){
									if(js.length>1) return 1;
									return game.hasPlayer(function(current){return current.countCards('he')})?1:0;
								}
								return 0;
							}
							for(var i=0;i<es.length;i++) minNum2=Math.min(minNum2,es[i].number);
							if(es.length){
								if(player.hasCard(function(card){return card.number>minNum2&&get.value(card)<9},'he')) return 1;
								if(!dngr){
		 ...
		 jlsg_yaoming:{
			...
			group:['jlsg_yaoming_strg','jlsg_yaoming_clear','jlsg_yaoming1','jlsg_yaoming2','jlsg_yaoming3'],
			subSkill:{
				strg:{
					trigger:{player:'useCard'},
		...
		jlsg_yaoming1:{
			...
			content:function(){
				...
				"step 1"
				if(result.bool){
					player.logSkill('jlsg_yaoming',result.targets);
					event.target=result.targets[0];
		...
		jlsg_yaoming2:{
			...
			usable:1,
			direct:true,
			check:function(event,player){return player.canMoveCard(true)},
			content:function(){
			    "step 0"
				player.moveCard();
				"step 1"
				player.logSkill('jlsg_yaoming');
			}
		},
		jlsg_yaoming3:{
			audio:"ext:民间极略:2",
			...
			content:function(){
				...
				if(result.bool){
					...
					jlsg_zhoufu:{
						...
						content:function(){
							'step 0'
							player.chooseToDiscard('h', '咒缚', '弃置1张手牌，令其进行判定', true).ai = function (card) {
								return 6-get.value(card);
							};
							'step 1'
							if(result.bool){
					...
					jlsg_tuodao:{
						...
						content:function(){
							...
							'step 1'
							player.chooseToUse({name:'sha'},'拖刀：是否对'+get.translation(trigger.player)+'使用1张【杀】？',trigger.player,-1).set('logSkill','jlsg_tuodao');
							'step 2'
					...
					jlsg_hemeng:{
						...
						subSkill:{
							usable:{
								...
								forced:true,
								content:function(){
									player.storage.jlsg_hemeng_usable=player.maxHp-player.hp+1;
								}
							}
						},
						ai:{
					...
					jlsg_sujian:{
						...
						content:function(){
							...
							player.chooseTarget('素检：选择1名其他角色弃置你1张牌，然后你弃置其1张牌',function(card,player,target){
								return player!=target&&target.countCards('he')>0;
							}).ai=function(target){
					...
					jlsg_shejian:{
						...
						content:function(){
							...
							target.chooseBool('是否对'+get.translation(player)+'使用1张【杀】？').ai=function(card,player,target){
					...
					jlsg_kuangao:{
						...
						content:function(){
							...
							player.chooseControl('选项一','选项二',function(){
									...
									return '选项二';
								}).set('prompt','狂傲<br><br><div class="text">1:弃置所有牌(至少1张),然后'+get.translation(trigger.player)+'弃置所有牌.</div><br><div class="text">2:令'+get.translation(trigger.player)+'将手牌补至其体力上限的张数(至多5张).</div></br>');
							'step 1'
							if(result.control=='选项一'){
					...
					jlsg_weifeng:{
						...
						content:function(){
							...
							player.chooseTarget('是否发动【威风】？',function(card,player,target){
								return player!=target&&target.countCards('h')&&10-get.value(card);
							}).ai=function(target){
					...
					jlsg_xieli:{
						...
						filter:function(event,player){
							if(!player.hasZhuSkill('jlsg_xieli')) return false;
							return true;
						},
						check:function(){return 1},
						content:function(){
							...
							if(event.targets.length){
								...
								if(current.group=='wu'&&current.num('h')){
									current.chooseCard('是否帮'+get.translation(player)+'打出1张拼点牌？').ai=function(card){
					...
					jlsg_yalv:{
						...
						content:function(){
							...
							player.chooseBool('是否摸1张牌？').ai=function(){
								return 1;
							}
							'step 2'
							if(result.bool){
					...
					jlsg_zhejie:{
						...
						content:function(){
							...
							if(get.type(result.cards[0])=='equip'){
								event.card=result.cards[0];
								player.chooseTarget('选择1名目标获得'+get.translation(event.card),function(card,player,target){
					...
					jlsg_fengya:{
						...
						content:function(){
							"step 0"
							player.draw();
							trigger.source.chooseBool('是否摸1张牌并令此伤害-1?').ai=function(){
								...
								if(player.isHealthy()){
									if(!hasTrick){
										if(player.hp>=player.countCards('h')){
											return player.hasCard(function(card){return get.tag(card,'damage')})?0:result;
										}else{
											return player.hasCard(function(card){return get.tag(card,'damage')&&card.name!='sha';})?0:result;
										}
									}
								}else{
					...
					jlsg_liuma:{
						...
						content:function(){
							...
							event.target.chooseCardTarget({
								prompt:'选择1名角色将你的1张装备牌交给该角色,或令'+get.translation(player)+'获得你1张手牌',
								filterCard:true,
								position:'e',
								filterTarget:function(card,player,target){
									return player!=target;
        ...
        return target.countCards('h')>0;
						...
						content:function(){
							...
							if(event.num<event.targets.length){
								...
								if(event.target.countCards('he')>=2){
									event.target.chooseCard('交给'+get.translation(player)+'1张牌，或弃置2张牌对其造成1点伤害','he').ai=function(card){
										if(get.attitude(event.target,player)>0) return 10-get.value(card);
										return 0;
									}
								}
								else if(event.target.countCards('h')==1){
									event.target.chooseCard('交给'+get.translation(player)+'1张牌','h',true);
								}
								else{
									event.num++;
									event.redo();
					...
					jlsg_zhongyong:{
						...
						subSkill:{
							...
							giveCard:{
								...
								content:function(){
									'step 0'
									player.chooseTarget('是否发动【忠勇】让1名角色获得你本回合的弃牌？',function(card,player,target){
										return player!=target;
									}).ai=function(target){
					...
					jlsg_bozhan:{
						...
						forced:true,
						content:function(){
							trigger.target.chooseToUse('是否对'+get.translation(trigger.player)+'使用1张【杀】？',{name:'sha'},-1,trigger.player);
						}
					},
					jlsg_qingxi:{
						audio:"ext:民间极略:true",
						...
						content:function(){
							...
							if(result.bool){
								...
								player.chooseControl('选项一','选项二','选项三',function(){
									return Math.random()<0.5?'选项一':'选项二';
								}).set('prompt','戟舞<br><br><div class="text">1:此【杀】不计入次数限制</div><br><div class="text">2:此【杀】无距离限制,且可以额外指定1个目标</div></br><div class="text">3:此【杀】的伤害值+1。</br>');
							}
							else{
								event.finish();
							}
					...
					jlsg_daoshi:{
						...
						content:function(){
							'step 0'
							trigger.player.chooseBool('是否发动【刀侍】摸1张牌并将装备区的1张牌交给'+get.translation(player)).ai=function(){
							...
							if(result.bool){
								...
								if(trigger.player!=player){
									trigger.player.chooseCardButton('选择1张牌交给'+get.translation(player),trigger.player.get('e'),true);
								}
								else{
									event.finish();
								}
					...
					jlsg_lirang:{
						...
						content:function(){
							'step 0'
							var next=trigger.player.chooseCard(get.prompt('jlsg_lirang',player));
							...
							next.ai=function(card){
								if(get.attitude(trigger.player,player)>0){
									if(sgs.needKongcheng(trigger.player)) return 20-get.value(card);
									return 7-get.value(card);
								}
								if(get.attitude(trigger.player,player)<=0){
									return card.name=='du';
					...
					jlsg_xianshi:{
						...
						content:function(){
							'step 0'
							trigger.source.chooseToDiscard('弃置1张牌并展示所有手牌，或令此伤害-1').ai=function(card){
					 ...
					 audio:"ext:民间极略:2",
						...
						check:function(event,player){
							if(player.isTurnedOver()){
								if(sgs.isFriend(player,event.player)){
									return true;
								}
								if(player.countCards('h','du')>2&&!sgs.isFriend(player,event.player)) return true;
							}
							return false;
						},
						content:function(){
					...
					jlsg_midao:{
						...
						content:function(){
							...
							'step 1'
							if(event.target.countCards('h')){
								event.target.chooseCard('选择1张手牌交给'+get.translation(player),true).ai=function(card){
									return -get.value(card);
								}
							}
							else{
					...
					jlsg_pudu:{
						...
						content:function(){
							...
							if(maxh){
								player.chooseCard('选择1张手牌交给'+get.translation(event.current),true).ai=function(card){
								...
								content:function(){
									...
									if(get.color(event.card)=='red'){
										...
										if(cards.length){
											if(cards.length==2){
												event.cards=cards;
												player.chooseToDiscard('纵情:选择1张牌弃置',function(card){
					...
					jlsg_bugua:{
						...
						content:function(){
							...
							event.chosed=false;
							'step 1'
							player.chooseCard('是否将1张手牌置于牌堆顶？').set('ai',function(card){
								var trigger=_status.event.getTrigger();
								var player=_status.event.player;
							...
							if(result&&result.cards){
								...
								if(trigger.player==player){
									...
									trigger.player.chooseCard('将1张手牌置于牌堆顶？').set('ai',function(card){
										var trigger=_status.event.getTrigger();
					...
					jlsg_bugua2:{
						...
						prompt:function(event,player){
							var str='';
							str+='是否对'+get.translation(event.player)+'发动【卜卦】令其摸1张牌';
							return str;
						},
						content:function(){
					...
					jlsg_bugua3:{
						...
						prompt:function(event,player){
							var str='';
							str+='是否对'+get.translation(event.player)+'发动【卜卦】令其弃1张牌';
							return str;
						},
						content:function(){
    ...
    content:function (){},
					...
					jlsg_huanbing:{
						...
						content:function(){
							...
							player.storage.jlsg_huanbing=player.storage.jlsg_huanbing.concat(trigger.card);
					...
					jlsg_huanbing2:{
						...
						content:function(){
							...
							if(player.storage.jlsg_huanbing.length){
								player.$phaseJudge(player.storage.jlsg_huanbing[0]);
								player.judge(function(card){
									if(get.color(card)=='red') return 1;
									return -0.5;
								});
							...
							player.gain(player.storage.jlsg_huanbing[0],'gain2');
					...
					jlsg_old_zhishi:{
						...
						prompt:"选择1张【杀】或【闪】，并且选择1名有手牌的其他角色，发动【治世】。",
						filterTarget:function (card,player,target){
						...
						content:function () {
							...
							if (cards[0].name == 'sha') {
								target.chooseToDiscard("请弃置1张【杀】，令" + get.translation(target) + "恢复1点体力，否则你受到1点伤害", { name: "sha" }).set("ai", function () {
									if (_status.nono == true) return false;
									return true;
								}).set('nono', nono);
							}
							if (cards[0].name == 'shan') {
								target.chooseCard("请展示1张【闪】，令" + get.translation(target) + "恢复1点体力，否则你受到1点伤害", 'h', function (card, player, target) {
					...
					jlsg_qianhuan:{
						...
						content:function(){
							...
							event.list=list;
							var list=[];
							for(var i=0;i<player.storage.jlsg_qianhuan_fenpei.length;i++){
								list.push(player.storage.jlsg_qianhuan_fenpei[i]);
							}
							if(list.length){
								for(var i=0;i<list.length;i++){	
							...
							if(event.isMine()){
								var slist=player.storage.fenpei.owned;
								event.dialog=ui.create.dialog('千幻：选择获得技能',[event.list,'character'],true);	
								...
								event.clickControl=function(link){									
									if(link=='cancel'){
									...
									else if(!player.skills.contains(link)){
										for(var i=0;i<this.childNodes.length;i++){
											if(this.childNodes[i].link==link&&this.childNodes[i].style.opacity!=0.4){
												event.num--;
												player.addSkill(link);
												player.popup(link);										
												player.storage.jlsg_qianhuan_fenpei=player.storage.jlsg_qianhuan_fenpei.concat(link);
												this.childNodes[i].style.opacity=0.5;
												break;
											}
										}										
									}
										...
										for(var i=0;i<player.storage.jlsg_qianhuan_fenpei.length;i++){
											if(player.storage.jlsg_qianhuan_fenpei[i]==link&&bool){
												player.storage.jlsg_qianhuan_fenpei.splice(i--,1);
											}
										}										
									}
									var list=[];
									for(var i=0;i<event.list.length;i++){
										...
										for(var j=0;j<skills.length;j++){
											...
											if(lib.translate[skills[j]+'_info']){
												...
												if(info.zhuSkill&&(str.indexOf('限定技')!=-1||str.indexOf('觉醒技')!=-1))  continue;
												if(str.indexOf('限定技')==0||str.indexOf('觉醒技')==0)  continue;
											}
											list.push(skills[j]);
										}
									}							
									if(event.num==0||!list.length){									
										ui.auto.show();
										event.dialog.close();
										event.control.close();
										game.resume();	
										var list2=[];
										for(var i=0;i<player.storage.jlsg_qianhuan_fenpei.length;i++){
											list2.push(player.storage.jlsg_qianhuan_fenpei[i]);
										}
										if(list2.length){
											(function(){
												var skills=list2;
												...
												var name='gameStart';
												if(event.triggername=='phaseBegin') var name2='gameDrawAfter';
													...
													if(trigger){
														...
														if(player==triggerevent.player&&trigger.player){
															if(typeof trigger.player=='string'){
																...
																if(name2&&trigger.player==name2) addx=true;
															}
															else if(trigger.player.contains(name)) add=true;
															else if(name2&&trigger.player.contains(name2)) addx=true;
														}
														if(trigger.global){
															if(typeof trigger.global=='string'){
																if(trigger.global==name) add=true;
																if(name2&&trigger.global==name2) addx=true;
															}
															else if(trigger.global.contains(name)) add=true;
															else if(name2&&trigger.global.contains(name2)) addx=true;
														}
														if(add&&player.isOut()==false) list.push(skills[i]);
														if(addx&&player.isOut()==false) list3.push(skills[i]);
													}
												}
												for(var i=0;i<list.length;i++){
													game.createTrigger('gameStart',list[i],player,triggerevent);
												}
												if(list3.length){
													for(var i=0;i<list3.length;i++){
														game.createTrigger('gameDrawAfter',list3[i],player,triggerevent);
													}										
												}
											}());								
										}					
									...
									if(button.classList.contains('selected')){
										...
										for(var i=0;i<event.control.childNodes.length;i++){
											for(var j=0;j<skills2.length;j++){
												...
												if(lib.translate[skills2[j]+'_info']&&event.control.childNodes[i].link==skills2[j]){
													var str=lib.translate[skills2[j]+'_info'];
													if(info.zhuSkill&&(str.indexOf('限定技')!=-1||str.indexOf('觉醒技')!=-1)){
														event.control.childNodes[i].style.opacity=0.4;
													}
													else if(str.indexOf('限定技')==0||str.indexOf('觉醒技')==0){
														event.control.childNodes[i].style.opacity=0.4;
													}
													else if(player.skills.contains(skills2[j])){
														event.control.childNodes[i].style.opacity=0.5;
													}
												}												
											}
										}
								...
								players.remove(player.name);
								for(var i=0;i<players.length;i++){
									var skills=lib.character[players[i]][3].slice(0);
									for(var j=0;j<skills.length;j++){
										var info=lib.skill[skills[j]];
										...
										if(lib.translate[skills[j]+'_info']){
											var str=lib.translate[skills[j]+'_info'];
											if(info.zhuSkill&&(str.indexOf('限定技')!=-1||str.indexOf('觉醒技')!=-1)) skills.splice(j--,1);
											if(str.indexOf('限定技')==0||str.indexOf('觉醒技')==0) skills.splice(j--,1);
										}											
									}
									skills2=skills2.concat(skills);
								}	
								while(event.num>0){
									var skills3=skills2.randomGet();
									if(!player.skills.contains(skills3)){
										event.num--;
										player.popup(skills3);
										player.addSkill(skills3);
										player.storage.jlsg_qianhuan_fenpei=player.storage.jlsg_qianhuan_fenpei.concat(skills3);
									}
								}	
								var list2=[];
								for(var i=0;i<player.storage.jlsg_qianhuan_fenpei.length;i++){
					...
					jlsg_jinglun:{
						...
						content:function(){
							'step 0'
							event.card=sgs.findCardInCardPile('wuxie');
							'step 1'
							if(event.card){
								...
								event.card=sgs.findCardInDiscardPile('wuxie');
								if(event.card){
					...
					jlsg_ruzong:{
						...
						chooseButton:{
							...
							prompt:function(links,player){
								return '将1张无懈可击当'+get.translation(links[0][2])+'使用';
							}
						},
						ai:{
							order:6,
					...
					jlsg_gongshen:{
						...
						check:function(card,event){
							if(sgs.needKongcheng(_status.event.player)) return 10-get.value(card)
							return 6-get.value(card);
						},
						content:function(){
							'step 0'
					...
					jlsg_jianyue:{
						...
						logTarget:'player',
						check:function(event,player){
							if(sgs.isFriend(player,event.player)) return !sgs.needKongcheng(event.player,true);
							return get.attitude(player,event.player)>0;
						},
						content:function(){
							'step 0'
							...
							if(isLess){
								var card=sgs.findCardInDiscardPile();
								if(card){
					...
					jlsg_pengri:{
						...
						content:function(){
							...
							player.draw(2);
							'setp 1'
							target.chooseToUse('是否对'+get.translation(player)+'使用1张【杀】？',{name:'sha'},player,-1);
						},
						ai:{
							order:9,
							result:{
								player:function(player){
									...
									for(var shot in shotter){
										if(get.distance(shot,player,'attack')<=1&&!sgs.isKongcheng(shot)&&!sgs.isFriend(shot,player)){
											sha++;
										}
									}
									var shan=sgs.getCardsNum('shan',player,player);
					...
					jlsg_fushe:{
						...
						content:function(){
							...
							trigger.player.addTempSkill('jlsg_fushe_scanning2','phaseUseAfter');
							"step 2"
							trigger.player.addTempSkill('jlsg_fushe_buff','phaseUseAfter');
						},
						subSkill:{
							scanning:{
								audio:false,
								...
								filter:function(event,player){
									if(event.cards){
										for(var i=0;i<event.cards.length;i++){
											if(get.position(event.cards[i])=='d'&&get.suit(event.cards[i])+'2'==player.storage.jlsg_fushe) 
												return true;
										}
									}
									return false;
							...
							buff:{
								...
								intro:{
									content:function(storage,player){       
										...
										return '该角色出牌阶段→'+get.translation(player.storage.jlsg_fushe) + '←花色的牌进入弃牌堆时，此阶段结束时受到来自SK张任的1点伤害';
									}
								},
								ai:{
									effect:{
										player:function(card,player,target){
											var zhangren=sgs.findPlayerBySkillName('jlsg_fushe');
							...
							scanning2:{
								...
								filter:function(event,player){
									...
									return (get.suit(event.result.card)+'2'==player.storage.jlsg_fushe);
								},
								content:function(){
									player.addTempSkill('jlsg_fushe_debuff','phaseUseAfter');
								}
					...
					jlsg_hengjiang:{
						...
						content:function(){
							...
							player.chooseControl('手牌上限+1','手牌上限-1').set('ai',function(event,player){
								if(sgs.isWeak(player)&&player.getHandcardLimit()<player.countCards('h')) return '手牌上限+1';
								var friends=sgs.getFriends(player);
								var needToThrowJudge=false;
								for(var i=0;i<friends.length;i++){
									if(friends[i].num('j')&&!friends[i].num('j','shandian')){
										needToThrowJudge=true;break;
									}
									else if(friends[i].num('j','shandian')){
										var rejudge=game.hasPlayer(function(target){
											return target.hasSkills(sgs.ai.skill.rejudge)&&sgs.isEnemy(player,target);
										});
										if(rejudge){
											needToThrowJudge=true;break;
										}
								...
								if(needToThrowJudge&&!sgs.isWeak(player)) return '手牌上限-1';
								var diren=sgs.getEnemies(player);
								var needToThrowEquip=false;
								for(var i=0;i<diren.length;i++){
									if(diren[i].num('e')){
										needToThrowEquip=true;break;
									}
								...
								if(needToThrowEquip&&!sgs.isWeak(player)) return '手牌上限-1';
								return '手牌上限+1';
							});
							'step 1'
								...
								content:function(){
									...
									if(event.count>0){
										player.chooseTarget(get.prompt('jlsg_hengjiang'),function(card,player,target){
											...
											if(sgs.isFriend(player,target)){
												if(target.countCards('j')&&!target.countCards('j','shandian')) return 8;
												var rejudge=game.hasPlayer(function(target1){
													return target1.hasSkills(sgs.ai.skill.rejudge)&&sgs.isEnemy(player,target1);
												});
												if(target.countCards('j','shandian')&&rejudge) return 10;
												return 0;
											}
											if(sgs.isEnemy(player,target)){
												var rejudge=game.hasPlayer(function(target1){
													return target1.hasSkills(sgs.ai.skill.rejudge)&&sgs.isEnemy(player,target1);
												});
												if(rejudge&&target.countCards('j','shandian')) return 7;
												if(target.countCards('e')&&!target.hasSkills(sgs.ai.skill.lose_equip)) return 6;
												return 0;
											}
											return 0;
										});
						...
						content:function(){
							...
							player.chooseTarget(get.prompt('jlsg_zhuanshan')).ai=function(target){
								if(target==player){
									if(target.countCards('j')){
										if(target.countCards('j','shandian')==0){
											if(event.triggername=='phaseBegin'){
												...
												if(sgs.isFriend(target,target.next)){
													return 5;
												}
												return -5;
											}
											...
											var bool=game.hasPlayer(function(target){
												return target.hasSkills(sgs.ai.skill.rejudge);
											});
											if(bool){
												return 5;
											}
									...
									else if(target.hasSkills(sgs.ai.skill.lose_equip)&&target.countCards('e')){
										return 5;
									}
									else{
										return -1;
									...
									if(att>0&&target.countCards('j')){
										if(event.triggername=='phaseBegin'){
											...
											if(sgs.isFriend(player,player.next)){
												return 6;
											}
											return -1;
										}
							...
							if(event.card){
								event.card.fix();
								ui.cardPile.insertBefore(event.card,ui.cardPile.firstChild);
								game.log(player,'将',event.target,'的1张牌置于牌堆顶');
							}
						}
					},
					jlsg_zhenlie:{
						...
						content:function(){
							...
							if(player.countCards('he')){
								player.chooseToDiscard('你可以弃置1张牌，令'+get.translation(trigger.player)+'展示所有手牌并弃置与之花色相同的牌，若不如此做，其失去1点体力','he').set('ai',function(card){
									if(sgs.isFriend(player,trigger.player)) return false;
									if(sgs.isWeak(player)) return false;
									if(sgs.isWeak(trigger.player)) return 10-get.value(card);
									return 6-get.value(card);
								});
							}
							else{
								...
								if(trigger.player.countCards('h')>0){
									...
									if(trigger.player.countCards('h',{suit:get.suit(result.cards[0])})){
										...
										trigger.player.chooseBool('是否弃置所有'+get.translation(get.suit(result.cards[0]))+'花色的手牌，否则失去1点体力').ai=function(){
											if(sgs.isWeak(trigger.player)) return true;
					...
					jlsg_miji:{
						...
						content:function(){
							...
							'step 1'
							var card=sgs.findCardInCardPile(function(card){
								return get.type(card)==result.control;
							});
							if(card){
								event.card1=card;
								player.showCards('秘计',event.card1);
								player.chooseTarget('将'+get.translation(card)+'交给1名角色').set('ai',function(target){
									var att=get.attitude(_status.event.player,target);
					...
					translate:{
						...
						jlsg_zhoufu_info:'其他角色的回合开始时，你可以弃置1张手牌，令其判定，若结果为黑桃，其于本回合内失去其所有技能；若结果为梅花，其弃置2张牌。',
						jlsg_yingbing_info:'每回合限1次，当1名其他角色的黑色判定牌生效后，你可以视为对其使用1张【杀】。',
						jlsg_tuodao_info:'每当你用【闪】抵消了1次【杀】的效果时，若使用者在你的攻击范围内，你可以立刻对其使用1张【杀】，此【杀】无视防具且不可被【闪】响应',
						jlsg_danqi_info:'觉醒技，回合开始阶段，若你的手牌数大于你的体力值，你须自减1点体力上限，恢复2点体力，并永久获得技能〈拖刀〉。',
						jlsg_zhenlie_info:'当你成为其他角色使用的【杀】或非延时锦囊牌的目标时，你可以失去1点体力，令此牌对你无效，然后你可以弃置1张牌，令该角色展示所有手牌并弃置与之花色相同的牌，若不如此做，其失去1点体力。',
						jlsg_miji_info:'回合开始阶段开始时，若你已受伤，你可以声明1种牌的类别，然后从牌堆随机亮出1张此类别的牌，将之交给1名角色。回合结束阶段开始时，若你的体力为全场最少（或之1），你亦可以如此做。',
						jlsg_pengri_info:'出牌阶段限1次，你可以摸2张牌，然后攻击范围内含有你的其他角色可依次对你使用1张【杀】',
						jlsg_danmou_info:'当你受到伤害后，你可以与伤害来源交换手牌。',
						jlsg_fushe_info:'其他角色的出牌阶段开始时，若其在你的攻击范围内，你可以选择1种花色。若如此做，此阶段结束时，若其有此花色的牌进入弃牌堆，你对其造成1点伤害，然后摸1张牌。',
						jlsg_ziguo_info:'出牌阶段限1次，你可以令1名已受伤的角色摸2张牌，若如此做，本回合你的手牌上限-2。',
						jlsg_shangdao_info:'锁定技，1名角色的准备阶段开始时，若其手牌数大于你，你展示牌堆顶牌并获得之。',
						jlsg_hengjiang_info:'弃牌阶段开始时，你可以令你的手牌上限+1或-1，若如此做，此阶段结束时，你可以弃置场上的至多X张牌（X为此阶段你弃置的牌数）。',
						jlsg_zhuanshan_info:'回合开始/结束阶段开始时，你可以令1名角色摸1张牌，然后将该角色的1张牌置于牌堆顶。',
						jlsg_hemeng_info:'出牌阶段，若你有手牌，可令1名其他角色观看你的手牌并获得其中1张，然后你观看该角色的手牌并获得其1张牌。每阶段限X+1次，X为你此阶段开始时已损失的体力值。',
						jlsg_sujian_info:'每当你从其他角色处获得1次牌时，可令1名其他角色弃置你1张牌，然后你弃置其1张牌。',
						jlsg_yexi_info:'回合结束阶段，你可以多弃1张手牌， 然后指定你以外的1个角色选择:1.使用黑色【杀】时无视防具。2.使用红色【杀】时无视距离。该角色将在他的下个出牌阶段得到上述效果中的1个。',
						jlsg_kuangyan_info:'锁定技，你受到1点无属性伤害时，该伤害对你无效，你受到2点或2点以上伤害时，该伤害+1。',
						jlsg_chaochen_info:'出牌阶段限1次，你可以将至少1张手牌交给1名其他角色，若如此做，该角色的回合开始阶段开始时，若其手牌数大于其体力值，你对其造成1点伤害。',
						jlsg_quanzheng_info:'当你成为其他角色使用的【杀】或非延时类锦囊牌的目标后，若其手牌或装备区的牌数大于你对应的区域，你可以摸1张牌。',
						jlsg_shejian_info:'出牌阶段对每名其他角色限1次，若你未装备防具，你可以弃置1名其他角色的1张牌，然后该角色可以视为对你使用1张【杀】。',
						jlsg_kuangao_info:'当1张对你使用的【杀】结算后，你可以选择1项：弃置所有牌（至少1张），然后该【杀】的使用者弃置所有牌；或令该【杀】的使用者将手牌补至其体力上限的张数（至多5张）。',
						jlsg_yinbing_info:'你攻击范围内的1名其他角色成为【杀】的目标时，你可以获得其装备区的1张牌，然后将该【杀】转移给你（你不得是此【杀】的使用者）；当你成为【杀】的目标时，你可以弃置1张牌，然后摸X张牌（X为你已损失的体力值）。',
						jlsg_fenwei_info:'当你使用【杀】对目标角色造成伤害时，你可以展示该角色的1张手牌：若为【桃】或【酒】，则你获得之；若不为基本牌，则你弃掉该牌并令该伤害+1。',
						jlsg_shiyong_info:'锁定技，你每受到1次红色【杀】或【酒】【杀】造成的伤害后，减少1点体力上限。',
						jlsg_angyang_info:'每当你指定目标后或成为目标后1张【决斗】或红色的【杀】时，你可以摸1张牌，若对方判定区内有牌，你可以摸2张牌。',
						jlsg_weifeng_info:'回合开始阶段，若你的手牌数小于你的体力值，你可以与1名角色拼点，若你赢，你从牌堆摸2张牌，若你没赢，该角色从牌堆摸2张牌。',
						jlsg_xieli_info:'主公技，当你需要打出1张拼点牌时，你可请场上吴将帮你出，所有吴将给出牌后，你必须从中挑选1张作为拼点牌并弃掉其余。',
						...
						jlsg_suiji_info:'其他角色的弃牌阶段开始时，你可以交给其至少1张手牌，然后其将超出其体力值数量的手牌交给你。',
						jlsg_fengyi_info:'当你成为非延时类锦囊牌的唯1目标后，你可以摸1张牌。',
						jlsg_yalv_info:'当你受到伤害后，或出牌阶段开始时，你可以观看牌堆顶2张牌并以任意顺序置于牌堆顶，然后你可以摸1张牌。',
						jlsg_xiemu_info:'1名角色的回合开始阶段开始时，你可以将1张牌置于牌堆顶，若如此做，该角色回合结束阶段开始时，你可以令其摸1张牌。',
						jlsg_zhejie_info:'其他角色的弃牌阶段结束时，你可以弃置1张手牌，令其弃置1张牌，若该角色弃置的牌为装备牌，你将之交给除该角色外的1名角色。',
						jlsg_fengya_info:'每当你受到1次伤害时，你可以摸1张牌，然后伤害来源可以摸1张牌并令此伤害-1。',
						jlsg_yijian_info:'你可以跳过你的出牌阶段并令1名其他角色摸1张牌，然后若该角色的手牌数不小于你的手牌数，你恢复1点体力。',
						jlsg_feijun_info:'锁定技，出牌阶段开始时，若你的手牌数不小于你的体力值，本阶段你的攻击范围+X且可以额外使用1张【杀】（X为你当前体力值）；若你的手牌数小于你的体力值，你不能使用【杀】直到回合结束。',
						jlsg_muniu_info:'你的回合内，当任意角色装备区的牌发生1次变动时，你可以选择1名角色并选择1项： 弃置其1张手牌，或令其摸1张牌。',
						jlsg_liuma_info:'出牌阶段限1次，你可以弃置1张基本牌，然后令至多2名至少1名装备区有牌的其他角色依次选择1项：将其装备区的1张牌交给1名其他角色，或你获得其1张手牌。',
						jlsg_baozheng_info:'锁定技，回合结束阶段开始时，你令其他有手牌的角色依次选择1项：交给你1张手牌；或弃置2张牌，然后对你造成1点伤害。',
						jlsg_lingnu_info:'锁定技，回合结束时，若你于此回合受到2点或更多的伤害，你减1点体力上限，然后从其他角色处依次获得1张牌。',
						jlsg_zhongyong_info:'回合开始阶段开始时，你可以失去1点体力，若如此做，本回合的摸牌阶段，你可以额外摸x张牌（x为你已损失的体力值）；本回合的出牌阶段，你与其他角色的距离为1；本回合的弃牌阶段结束时，你可以令1名其他角色获得你本阶段弃置的牌。',
						jlsg_bozhan_info:'当你使用或被使用1张【杀】并完成结算后，若此【杀】未造成伤害，则此【杀】的目标或你可以对你或此【杀】的使用者使用1张【杀】(无距离限制）。',
						jlsg_qingxi_info:'锁定技，当你使用【杀】指定1个目标后，若你装备区的牌数少于该角色，则其不能使用【闪】响应此【杀】。',

						jlsg_danshou_info:'锁定技，当1名角色使用【杀】指定你为目标后，若你与其均有手牌，该角色须与你拼点，若你赢，你摸2张牌，然后弃置其1张牌；若你没赢，此【杀】不可被【闪】响应，你将摸1张牌。',
						jlsg_yonglie_info:'当你攻击范围内的1名角色受到【杀】造成的1次伤害后，你可以失去1点体力，然后对伤害来源造成1点伤害。',
						jlsg_hengshi_info:'弃牌阶段开始时，你可以摸等同于手牌数的牌。',
						jlsg_zhijiao_info:'限定技，回合结束阶段开始时，你可以令1名其他角色获得你的本回合因弃置而进入弃牌堆的牌。',
						jlsg_jiwux_info:'出牌阶段开始时，你可以展示1张【杀】，令其获得以下效果之1（进入牌堆后失效）：1、此【杀】不计入次数限制；2、此【杀】无距离限制，且可以额外指定1个目标；3、此【杀】的伤害值+1。',
						jlsg_daoshi_info:'1名角色的回合结束阶段开始时，若其装备区有牌，其可以摸1张牌，然后将其装备区的1张牌交给你。',
						jlsg_lirang_info:'1名角色的回合开始阶段结束时，其可以将1张与所有［礼］花色均不同的手牌置于你的武将牌上，成为［礼］，然后摸1张牌。你可以将2张［礼］当【桃】使用。',
						jlsg_xianshi_info:'每当你受到1次伤害时，你可以令伤害来源选择1项：展示所有手牌并弃置其中1张；或令此伤害-1.',
						jlsg_chengxiang_info:'每当你受到伤害后，你可以亮出牌顶堆的4张牌，然后获得其中的至少1张点数和不大于13的牌，将其余的牌置入弃牌堆。',
						jlsg_renxin_info:'每当1名其他角色处于濒死状态时，你可以翻面并将所有手牌交给该角色，令其恢复1点体力。',
						jlsg_midao_info:'出牌阶段限1次，你可以令手牌数大于你的其他角色依次交给你1张牌，然后若你的手牌数为全场最多，你失去1点体力。',
						jlsg_yishe_info:'出牌阶段限1次，你可以与1名手牌数不大于你的其他角色交换手牌。',
						jlsg_pudu_info:'限定技，出牌阶段，你可以获得所有其他角色的手牌，然后依次交给其他角色1张牌，直到你的手牌数不为全场最多。',
						jlsg_zongqing_info:'摸牌阶段开始时，你可以进行1次判定，若如此做，此阶段摸牌后你须展示之，然后弃置其中与该判定牌颜色不同的牌。若以此法弃置的牌为黑色，视为你使用1张【酒】；若以此法弃置的牌为红色，视为你使用1张【桃】。',
						...
						jlsg_zhihe_info:'出牌阶段限1次，你可以展示所有手牌，并将其中每种花色的牌弃置至1张，然后将手牌数翻倍。',
						jlsg_caijie_info:'其他角色的回合开始阶段开始时，若其手牌数不小于你，你可以与其拼点，若你赢，你摸2张牌；若你没赢，视为其对你使用1张【杀】。',
						jlsg_jilei_info:'当你受到伤害后，你可以令伤害来源展示所有手牌并弃置其中类别相同且最多（或之1）的所有牌。',
						jlsg_yanliang_info:'1名角色的回合开始阶段开始时，你可以弃置1张红色牌，令其本回合的摸牌阶段直接取消，出牌阶段后进行摸2张牌；或弃置1张黑色牌，令其本回合的摸牌阶段直接取消，弃牌阶段后进行摸2张牌。',
						jlsg_duzhi_info:'当你恢复体力后，你可以令1名其他角色失去X点体力，然后该角色可以对你使用1张【杀】；当你使用红色【杀】造成伤害后，你可以令至多X名其他角色失去1点体力，然后你选择的角色可以对你使用1张【杀】（X为当前体力的改变值）。',
						jlsg_lieyi_info:'锁定技，你的【桃】均视为【杀】；你的【闪】均视为【酒】。',
						jlsg_baoli_info:'出牌阶段限1次，你可以对1名装备区没有牌或判定区有牌的其他角色造成1点伤害。',
						jlsg_huanbing_info:'锁定技，当你成为【杀】的目标时，终止此【杀】的结算，改为将之置于你的武将牌上。回合开始阶段开始时，你须为你武将牌上的每1张【杀】进行1次判定：若结果为红色，你摸1张牌；若结果为黑色，你须失去1点体力。然后将【杀】收入手牌。',
						jlsg_hongyuan_info:'出牌阶段限1次，你可以弃掉2张手牌，将1名角色装备区的牌移动到另1名其他角色对应的区域（不可覆盖）。',
						jlsg_huaqiang_info:'出牌阶段限1次，你可以弃置X种不同花色的手牌，然后对1名其他角色造成1点伤害（X为你的体力值且至多为4）。',
						jlsg_chaohuang_info:'出牌阶段限1次，你可以失去1点体力，然后视为对你攻击范围内的任意名角色依次使用1张【杀】（不计入出牌阶段的使用限制）。',
						jlsg_huilian_info:'出牌阶段限1次，你可以令1名其他角色进行1次判定并获得生效后的判定牌。若结果为红桃，该角色恢复1点体力。',
						jlsg_wenliang_info:'1名角色的红色判定牌生效后，你可以摸1张牌。',
						jlsg_qianhuan_info:'变化技，锁定技，你的每个回合开始时，随机展示3张未上场且你拥有的武将，你获得其中的2个技能（觉醒技、主公技、限定技和变化技除外），直到你的下个回合开始。若该局游戏为双将模式，则移除你的另1名武将，将“2个”改为“4个”。',
						jlsg_jinglun_info:'当你使用非延时类锦囊牌时，若牌堆有【无懈可击】，你可以随机获得1张；若没有，你可以将1张【无懈可击】从弃牌堆随机置入牌堆，然后摸1张牌。',
						jlsg_ruzong_info:'你可以将【无懈可击】当任意基本牌使用。',
						jlsg_leiji_info:'当其他角色使用【闪】时，你可以将牌堆或弃牌堆里的1张【闪电】置入1名角色的判定区。',
						jlsg_shanxi_info:'锁定技，你不能成为【闪电】的目标，其他角色的【闪电】的判定牌生效后，你获得之。',
						jlsg_guhuo_info:'（没有〈缠怨〉技能）其他角色的回合开始时，你可以与其拼点：若你赢，可以视为使用1张非延迟锦囊牌；若你没赢，该角色对你造成1点伤害。',
						jlsg_fulu_info:'其他角色的回合结束时，你可以令其摸1张牌，然后若此回合内该角色对你造成过伤害，你令其获得〈缠怨〉（锁定技，当你受到伤害时，你令于吉恢复1点体力并失去〈缠怨〉；当于吉受到伤害时，你失去1点体力并失去〈缠怨〉）。',
						jlsg_gongshen_info:'出牌阶段，你可以弃置3张牌，然后摸1张牌，若此时你的手牌数为最少（或之1），你恢复1点体力。',
						jlsg_jianyue_info:'1名角色的回合结束阶段开始时，若该角色的手牌数为最少（或之1），你可以令其从弃牌堆随机获得牌直到其手牌数不为最少（或之1）。',
						jlsgsk_simashi:"司马师",
						jlsgsk_xianglang:"向朗",
						jlsgsk_luji:"陆绩",
						...
						jlsgsk_panfeng:"潘凤",
						jlsg_quanlue:"权略",
						jlsg_quanlue_info:"出牌阶段开始时，你可以展示所有手牌并选择其中1种花色的手牌，然后摸与之等量的牌。若如此做，此阶段结束时，你须展示手牌并弃置所有此花色的手牌。",
						jlsg_huaiju:"怀橘",
						jlsg_huntian:"浑天",
						jlsg_huaiju_info:"你的1个阶段结束时，若你的手牌数为3，你可以摸1张牌或弃置2张牌。",
						jlsg_huntian_info:"当你的牌因弃置而进入弃牌堆时，你可将其中任意张置于牌堆顶，然后从牌堆随机获得1张与这些牌类别均不同的牌。",
						jlsg_cangshu:"藏书",
						jlsg_kanwu:"勘误",
						jlsg_kanwu1:"勘误·闪",
						jlsg_kanwu2:"勘误·桃",
						...
						jlsg_cangshu_info:"当其他角色使用非延时类锦囊牌时，你可以交给其1张基本牌，然后获得此牌并令其无效。",
						jlsg_kanwu_info:"当你于回合外需要使用或打出1张基本牌时，你可以弃置1张锦囊牌，视为使用或打出之。",
						jlsg_huage:"化戈",
						jlsg_muyi:"母仪",
						jlsg_huage_info:"出牌阶段限1次，你可以令所有角色依次弃置至少1张牌，目标角色每弃置1张【杀】则摸1张牌。",
						jlsg_muyi_info:"其他角色的回合开始阶段开始时，其可以交给你1至2张牌，然后此回合结束时，你交给其等量的牌。",
						jlsg_diezhang:"迭嶂",
						jlsg_diezhang2:"迭嶂",
						jlsg_diezhang3:"迭嶂",
						jlsg_diezhang_info:"出牌阶段，当你使用牌时，若此牌的点数大于本回合你上1张使用的牌，你可以摸1张牌。",
						jlsg_xiongyi2:"雄异·摸牌",
						jlsg_xiongyi1:"雄异·回血",
						jlsg_xiongyi:"雄异",
						jlsg_xiongyi_info:"锁定技，你的回合开始时，若你的体力值为1，你恢复1点体力；若你没有手牌，你摸2张牌。",
						jlsg_sijian:"死谏",
						jlsg_gangzhi:"刚直",
						...
						jlsg_zhige_3:"止戈·闪",
						jlsg_zhige_4:"止戈·杀",
						jlsg_yanxi_info:"回合开始阶段开始时或回合结束阶段开始时，若你的装备区内没有牌，你可以摸1张牌。",
						jlsg_zhige_info:"你可以弃置你装备区内的所有牌(至少一张)，视为使用一张【杀】或【闪】。",
						jlsg_shangyi:"尚义",
						jlsg_wangsi:"忘私",
						jlsg_shangyi_info:"出牌阶段限1次，你可以选择1名有手牌的其他角色，然后观看其所有类型的牌，并可以弃置其中1张黑色牌。",
						jlsg_wangsi_info:"当你受到伤害后，你可以观看伤害来源的所有类型的牌，并可以弃置其中1张红色牌。",
						jlsg_bibu:"裨补",
						jlsg_bibu1:"裨补·摸牌",
						jlsg_kuangzheng:"匡正",
						jlsg_bibu_info:"其他角色的回合结束时：若你的手牌数大于体力值，你可以将1张手牌交给1名其他角色；若你的手牌数不大于体力值，你可以摸1张牌。",
						jlsg_kuangzheng_info:"你的回合结束时，你可以将1名角色的武将牌恢复至游戏开始时的状态（即将其武将牌翻转至正面朝上并重置之）。",
						jlsg_duanlan:"断缆",
						jlsg_duanlan_info:"出牌阶段限1次，你可以弃置其他角色区域内的1至3张牌，然后选择1项： 1、失去1点体力;2、弃置1张大于这些牌点数之和的牌。",
						jlsg_yaoming:"邀名",
						jlsg_yaoming1:"邀名",
						jlsg_yaoming2:"邀名",
						jlsg_yaoming3:"邀名",
						jlsg_yaoming_info:"出牌阶段，当你使用或打出1张花色与本阶段皆不相同的牌时：第1种，你可以摸1张牌；第2种，你可以弃置1名其他角色的1张牌；第3种，你可以将场上1张牌移至另1位置；第4种，你可以对1名其他角色造成1点伤害。",
						jlsg_zhengyi:"整毅",
						jlsg_zhengyi_sha:"整毅·杀",
						jlsg_zhengyi_shan:"整毅·闪",
						jlsg_zhengyi_tao:"整毅·桃",
						jlsg_zhengyi_phaseUse:"整毅",
						jlsg_zhengyi_info:"你出牌阶段出牌时，若你的手牌数等于X，你可以视为使用任意一张基本牌，然后弃一张牌（X为你的体力值+1）；你的回合外，当你需要使用或打出一张基本牌时，若你的手牌数为X，则你可以摸一张牌并视为使用或打出了此牌。",
						jlsg_kuangfu:"狂斧",
						jlsg_kuangfu_info:"当你使用【杀】对目标角色造成伤害后，你可以获得其装备区里的1张牌。",	
						jlsg_jinglun:'经纶',
						jlsg_jinglun_info:'当你使用非延时类锦囊牌时，若牌堆有【无懈可击】，你可以随机获得1张；若没有，你可以将1张【无懈可击】从弃牌堆随机置入牌堆，然后摸1张牌。',
						jlsg_ruzong:'儒宗',
						jlsg_ruzong_info:'你可以将【无懈可击】当任意基本牌使用。',
				...
				if(jilvesanguo.forbidBtCharacter){
					var forbidList=[
						"jlsgsk_xianglang",
						"jlsgsk_luji",
						"jlsgsk_bianfuren",
					...
					for(var i=0;i<forbidList.length;i++){
						l2.push(jlsg_sk.translate[forbidList[i]]);
					}
					//game.print(l2);
					for(var i in jlsg_sk.character){
						if(forbidList.contains(i)){
							 delete jlsg_sk.character[i];
						}
					}
				}
				if(jilvesanguo.forbidBtCharacter){
                ...
                if(lib.device||lib.node){
                    for(var i in jlsg_sk.character){jlsg_sk.character[i][4].push('ext:民间极略/'+i+'.jpg');}
                }else{
                    for(var i in jlsg_sk.character){jlsg_sk.character[i][4].push('db:extension-民间极略:'+i+'.jpg');}
                }
                return jlsg_sk;
            });	
            lib.config.all.characters.push('jlsg_sk');
            ...
            game.import('character',function(){
                var jlsg_sr = {
                    ...
                    connect:true,	
                    character:{ 
						jlsgsr_zhangliao:['male','wei',4,['jlsg_wuwei','jlsg_yansha'],['die_audio']],
						jlsgsr_xiahoudun:['male','wei',4,['jlsg_zhonghou','jlsg_ganglie'],['die_audio']],
						jlsgsr_zhenji:['female','wei',3,['jlsg_liuyun','jlsg_lingbo','jlsg_qingcheng_zhu'],['die_audio']],
						jlsgsr_xuchu:['male','wei',4,['jlsg_aozhan','jlsg_huxiao'],['die_audio']],
						jlsgsr_simayi:['male','wei',3,['jlsg_guicai','jlsg_langgu','jlsg_zhuizun'],['die_audio']],
						jlsgsr_guojia:['male','wei',3,['jlsg_tianshang','jlsg_yiji','jlsg_huiqu'],['die_audio']],
						jlsgsr_caocao:['male','wei',4,['jlsg_zhaoxiang','jlsg_zhishi','jlsg_jianxiong'],['zhu','die_audio']],
						jlsgsr_zhaoyun:['male','shu',4,['jlsg_jiuzhu','jlsg_tuwei'],['die_audio']],
						jlsgsr_zhangfei:['male','shu',4,['jlsg_xujin','jlsg_paoxiao'],['die_audio']],
						jlsgsr_machao:['male','shu',4,['jlsg_benxi','jlsg_yaozhan'],['die_audio']],
						jlsgsr_guanyu:['male','shu',4,['jlsg_wenjiu','jlsg_shuixi'],['die_audio']],
						jlsgsr_zhugeliang:['male','shu',3,['jlsg_sanfen','jlsg_guanxing','jlsg_weiwo'],['die_audio']],
						jlsgsr_huangyueying:['female','shu',3,['jlsg_shouji','jlsg_hemou','jlsg_qicai'],['die_audio']],
						jlsgsr_liubei:['male','shu',4,['jlsg_rende','jlsg_chouxi','jlsg_yongbing'],['zhu','die_audio']],
						jlsgsr_sunshangxiang:['female','wu',3,['jlsg_yinmeng','jlsg_xiwu','jlsg_juelie'],['die_audio']],
						jlsgsr_daqiao:['female','wu',3,['jlsg_fangxin','jlsg_xiyu','jlsg_wanrou'],['die_audio']],
						jlsgsr_huanggai:['male','wu',4,['jlsg_zhouyan','jlsg_zhaxiang'],['die_audio']],
						jlsgsr_lvmeng:['male','wu',4,['jlsg_shixue','jlsg_guoshi'],['die_audio']],
						jlsgsr_zhouyu:['male','wu',3,['jlsg_yingcai','jlsg_weibao','jlsg_choulve'],['die_audio']],
						jlsgsr_ganning:['male','wu',4,['jlsg_jiexi','jlsg_youxia'],['die_audio']],
						jlsgsr_luxun:['male','wu',3,['jlsg_dailao','jlsg_youdi','jlsg_ruya'],['die_audio']],
						jlsgsr_sunquan:['male','wu',4,['jlsg_quanheng','jlsg_xionglve','jlsg_fuzheng'],['zhu','die_audio']],
						jlsgsr_lvbu:['male','qun',4,['jlsg_jiwu','jlsg_sheji'],['die_audio']],
						jlsgsr_huatuo:['male','qun',3,['jlsg_xingyi','jlsg_guagu','jlsg_wuqin'],['die_audio']],
						jlsgsr_diaochan:['female','qun',3,['jlsg_lijian','jlsg_manwu','jlsg_baiyue'],['die_audio']], 
						jlsgsr_shuangxiong:['male','qun',4,['jlsg_hubu','jlsg_langxing','jlsg_shuangxiong'],['die_audio']], 
					},
					characterIntro:{},
					skill:{
					...
					jlsg_wuwei:{
						...
						prompt:'是否发动技能【无畏】，展示牌中每有1张基本牌便可视为对1名角色使用1张【杀】（每名角色限一次）',
						content:function(){
							'step 0'
							...
							if(result.bool){
								...
								player.chooseTarget('请选择无畏的目标',function(card,player,target){
									...
									if(sgs.isEnemy(player,target)){
										return 10-sgs.getDefenseSha(target,player);
									}
									return false;
								});
							}
					...
					jlsg_yansha:{
						...
						subSkill:{
							cards:{
								...
								content:function(){
									'step 0'
									var next=player.chooseCard('选择1张牌置于武将牌上作为［掩］','h');
									next.ai=function(card){
										return 7-get.value(card);
									};
									'step 1'
					...
					jlsg_liuyun:{
						...
						prompt:'弃置1张黑色牌，令1名角色选择1项：恢复1点体力或摸2张牌',
						filterTarget:true,
						content:function(){
							player.link();
                ...
                if(card.nature=='poison') return 1;
						...
						content:function(){
							...
							player.chooseControl('收入手牌','置入弃牌堆',ui.create.dialog('战',player.storage.jlsg_aozhan)).ai=function(event,player){
								...
								for(i=0;i<cards.length;i++){
									...
									if(sgs.isWeak(player)&&get.tag(cards[i],'save')) value+=get.value(cards[i]);
								}
					...
					jlsg_guicai:{
						...
						content:function(){
							...
							get.translation(trigger.player.judging[0])+'，打出1张手牌代替之或亮出牌顶的1张牌代替之').set('ai',function(card){
					...
					jlsg_tianshang:{
						...
						content:function(){
							...
							player.chooseTarget('是否发动【天殇】？',function(card,player,target){
								...
								if(num>0){
									if(target.isDamaged()&&target.hasSkills(sgs.ai.skill.need_maxhp)) return 5;
									if(sgs.isWeak(target)) return 3;
									if(target.isDamaged()) return 2;
									return 1;
								}
								return 0;
					...
					jlsg_huiqu:{
						...
						content:function(){
							...
							var next=player.chooseToDiscard('弃置1张牌发动【慧觑】？')
							next.ai=function(card){
								if(check1){
							...
							if(result.color){
								...
								if(result.color=='red'){
									...
									player.chooseTarget('选择1名目标对其造成1点伤害，然后其摸1张牌。',true).ai=function(target){
										return get.damageEffect(target,player,player);
									}
								}
							}
					...
					jlsg_jiwu:{
						...
						content:function(){
							'step 0'
							if(cards[0]){
								player.discard(player.get('h').remove(cards[0]));
							}
							else{
								player.draw();
							}
					...
					jlsg_sheji:{
						...
						trigger:{global:'damageEnd'},
						filter:function(event,player){
							return player.countCards('he')>0&&event.source&&event.source.get('e','1')!=undefined&&event.source!=player;
						},
						check:function(event,player){
							return get.attitude(player,event.source)<=0;
						},
						...
						content:function(){
							'step 0'
							var next=player.chooseToDiscard('he','是否发动【射戟】？弃置1张牌获得'+get.translation(trigger.source)+'的'+get.translation(trigger.source.get('e','1'))+'?');
							next.logSkill=['jlsg_sheji',trigger.source];
							next.ai=function(card){
								if(sgs.isKongcheng(player)&&sgs.needKongcheng(player,true)) return false;
					...
					jlsg_xingyi:{
						...
						ai:{
							...
							result:{
								...
								target:function(player,target){
									if(sgs.needKongcheng(target)&&target.countCards('h')==1) return 5;
									if(target.countCards('h')>target.hp&&target.isDamaged()) return 4;	
									if(sgs.isWeak(target)) return 2;
									if(target.isDamaged()) return 1;
									if(!sgs.hasLoseHandcardEffective(target)&&target.isDamaged()) return 1;
									if(target.hp==sgs.getBestHp(target)) return -0.1;
									if(!target.isDamaged()&&sgs.hasLoseHandcardEffective(target)) return -1;
									return 0;
								}
							},
							threaten:2
					...
					jlsg_wuqin:{
						...
						content:function(){
							...
							player.chooseToDiscard('是否发动【五禽】？',function(card){
								...
								if(sgs.needKongcheng(player)&&player.countCards('h')==1) return 10-get.value(card);
							...
							if(result.bool){
								player.chooseBool('摸2张牌，或进行1个额外的出牌阶段。').ai=function(){
									...
									if(sgs.needKongcheng(player,true)) return false;
									return true;
								}
							}
							else{
					...
					jlsg_baiyue:{
						...
						subSkill:{
							...
							countJudge:{
								...
								content:function(){
									player.storage.jlsg_baiyue=player.storage.jlsg_baiyue.concat(trigger.result.card);
								}
							}
						},
						init:function(player){
		...
		                },
			...
			             content:function () {
				...
				       		}, '交给' + get.translation(trigger.player) + '2张基本牌',true).set('ai', function (card) {
					...
					            trigger.player.chooseBool('是否视为对' + get.translation(player) + '使用1张【杀】').set('ai', function (event, player) {
...
            		},
		...
		            },
					...
					jlsg_fangxin_old:{
						...
						chooseButton:{
							dialog:function(event,player){
								return ui.create.dialog('将1张梅花牌当【兵粮寸断】或将1张方片牌当【乐不思蜀】对自己使用，若如此做，视为你使用1张【桃】。',player.get('he'),'hidden');
							},
							filter:function(button,player){
					...
					jlsg_xiyu:{
						...
						content:function(){
							'step 0'
							player.chooseTarget('细语：弃置1名角色的1张牌，然后该角色进行1个额外的出牌阶段',function(card,player,target){
								return target.countCards('he')>0;
							}).ai=function(target){
					...
					jlsg_zhaxiang:{
							...
							if(player.countCards('h','sha')>player.getCardUsable('sha')||!game.hasPlayer(function(current){
								...
								if(card.name=='sha'){
									for(var i=0;i<game.players.length;i++){
										...
										var effect=get.effect(target,{name:'sha',nature:'fire'},player,player);
						...
						content:function(){
							...
							event.target.chooseCard('交给'+get.translation(player)+'一张牌，或展示并获得此牌。').ai=function(card){
								var effect=get.effect(event.target,{name:'sha',nature:'fire'},player,player);
								if(effect>0){
									if(att>0)return -1;
									if(player.hasShan()){
										return -1;
					...
					jlsg_guoshi:{
						...
						trigger:{global:'phaseEnd'},
						filter:function(event,player){
							return event.player.storage.jlsg_guoshi.length>0;
						},
						init:function(){
							for(var i=0;i<game.players.length;i++){
						...
						content:function(){
							'step 0'
							var att=get.attitude(player,trigger.player);
							player.chooseCardButton(trigger.player.storage.jlsg_guoshi,'是否对'+get.translation(trigger.player)+'发动【国士】？').ai=function(button){
								if(att>0) return 1;
								return 0;
							}
							'step 1'
							...
							'step 2'
							delete trigger.player.storage.jlsg_guoshi;
						},
						group:['jlsg_guoshi2'],
						...
						subSkill:{
							check:{
								...
								content:function(){
									...
									player.addTempSkill('jlsg_guoshi_judge','phaseAfter');
									player.addTempSkill('jlsg_guoshi_discard','phaseAfter');
								}
							},
							judge:{
								trigger:{global:'judgeAfter'},
								...
								content:function(){
									if(trigger.result.card)
										player.storage.jlsg_guoshi=player.storage.jlsg_guoshi.concat(trigger.result.card);
								}
							},
							discard:{
								...
								content:function(){
									...
									for(var i=0;i<trigger.cards.length;i++){
										if(get.position(trigger.cards[i])=='d'&&trigger.cards[i]){
											player.storage.jlsg_guoshi=player.storage.jlsg_guoshi.concat(trigger.cards[i]);
										}
									}
								}
							}
					...
					jlsg_dailao:{
						...
						prompt:"你可以将武将牌背面朝上并摸1张牌，视为对1名其他角色使用1张【杀】，然后弃置该目标的1张牌",
						check:function () {
							return 1;
						},
						onuse: function (result, player) {
					...
					jlsg_quanheng:{
						...
						chooseButton:{
							...
							prompt:function(links,player){
								return '至少1张手牌当'+get.translation(links[0][2])+'使用';
							},
						},
					...
					jlsg_fuzheng2:{
						...
						content:function(){
							...
							if(event.current&&event.current.num('h')){
								event.current.chooseCard('选择1张手牌置于牌堆顶','h',true);
							}
							else{
								event.goto(2);
							}
					...
					jlsg_jiuzhu:{
						...
						srlose:true,
						group:"jlsg_jiuzhu2",
						trigger:{global:['useCardAfter','discardAfter']},
						filter:function(event,player){
							for(var i=0;i<event.cards.length;i++){
						...
						content:function(){
							...
							var next=player.chooseToDiscard('是否发动【救主】替换弃牌堆中的'+get.translation(event.cards)+'?','he',[1,event.cards.length],function(card){
								return card.name!='shan';
							});
							next.ai=function(card){
								if(get.attitude(player,_status.currentPhase)<0&&get.effect(_status.currentPhase,{name:'sha'},player)>0){
									if(_status.currentPhase.hp<=2||_status.currentPhase.countCards('h')<=2)return 7-get.value(card);
								}
								if(player.countCards('h',{name:'shan'})>=1) return -1;
								return 5.1-get.value(card);
							};
							next.logSkill=['jlsg_jiuzhu',_status.currentPhase];
							'step 1'
							if(result.bool){
								var cards=[];
								for(var i=0;i<result.cards.length;i++){
									if(event.cards.length>i){
										cards.push(event.cards[i]);
									}
								}
								player.gain(cards,'gain2');
								if(_status.currentPhase!=player){
									player.chooseBool('是否对'+get.translation(_status.currentPhase)+'使用1张无视防具的杀？').ai=function(){
							...
							'step 2'
							if(result.bool){
								player.addTempSkill('jlsg_buff_chuantou','shaAfter');
					...
					jlsg_jiuzhu2:{
						...
						content:function(){
							'step 0'
							var next=player.chooseToDiscard('是否发动【救主】替换弃牌堆中的'+get.translation(trigger.result.card)+'?','he',function(card){
								return card.name!='shan';
							});
							next.ai=function(card){
								if(get.attitude(player,_status.currentPhase)<0&&get.effect(_status.currentPhase,{name:'sha'},player)>0){
					...
					jlsg_tuwei:{
						...
						content:function(){
							...
							player.chooseCardTarget({
								...
								filterTarget:function(card,player,target){
									return trigger.player==target||trigger.target==target&&target.countCards('he')>0;
								},
								selectTarget:[1,2],
								ai1:function(card){
									return 6-get.value(card);
					...
					jlsg_benxi:{
						...
						content:function(){
							"step 0"
							trigger.target.chooseToDiscard('请弃置1张装备牌，否则不能使用闪抵消此杀','he',function(card){
					...
					jlsg_wenjiu2:{
						...
						content:function(){
							'step 0'
							player.chooseCardButton('请弃置1张［酒］，该伤害+1点',true,player.storage.jlsg_wenjiu).ai=function(button){
						...
						content:function(){
							...
							if(result.bool){
								...
								switch(get.suit(result.cards[0])){
									case 'heart':trigger.player.addTempSkill('jlsg_hemou_heart','phaseAfter');break;
									case 'diamond':trigger.player.addTempSkill('jlsg_hemou_diamond','phaseAfter');break;
									case 'club':trigger.player.addTempSkill('jlsg_hemou_club','phaseAfter');break;
									case 'spade':trigger.player.addTempSkill('jlsg_hemou_spade','phaseAfter');break;
								}
							}
							else{
								event.finish();
					...
					jlsg_chouxi:{
						...
						content:function(){
							...
							event.dialog=ui.create.dialog('弃置1张与'+get.translation(player)+'弃置的牌类别均不同的牌,然后让'+get.translation(player)+'获得'+get.translation(event.cards1)+'或受到来自'+get.translation(player)+'的1点伤害并获得其中1种类别的牌.','hidden');
							...
							if(result.bool){
								...
								event.target.damage();
								var dialog=ui.create.dialog('仇袭：选择1张的卡牌获得之',event.cards1);
					...
					jlsg_zhaoxiang:{
						...
						content:function(){
							...
							if(get.distance(player,trigger.player,'attack')<=1){
								...
								next.ai=function(card){
									if(sgs.isFriend(player,trigger.player)){
										if(sgs.needKongcheng(trigger.player)&&trigger.player.countCards('h')==1) return 6-get.value(card);
										...
										if(sgs.isFriend(player,trigger.target)) return 6-get.value(card);
							...
							if(result.bool){
								...
								if(trigger.player.countCards('h')){
									trigger.player.chooseCard('交给'+get.translation(player)+'一张牌或令打出的杀无效化').set('ai',function(card){
					...
					jlsg_zhonghou:{
						...
						content:function (){
							'step 0' 
							game.log(trigger.player,'向',player,'请求使用1张',trigger.card);
							player.chooseBool('是否失去1点体力视为'+get.translation(trigger.player)+'使用1张'+get.translation(trigger.card)+'？',function(event,player){
							...
							'step 1'
							if(result.bool){
								game.log(player,'同意了',trigger.player,'使用1张',trigger.card);
								player.draw();
								player.loseHp(); 
								event.finish();
							}
				...
				translate:{
					...
					jlsg_shuangxiong_info:"你处于决斗状态时，你可以将任意1张牌当【杀】打出；当你的判定牌生效时：若为黑色，你从牌堆里摸1张【杀】；若为红色，你从牌堆里摸1张【闪】。",
					jlsg_hubu_info:"你每使用【杀】造成1次伤害后或受到1次其他角色使用【杀】造成的伤害后，可以令除你外的任意角色进行1次判定；若结果不为黑桃，则视为你对其使用1张【决斗】（此【决斗】不能被【无懈可击】响应）。",
					jlsg_langxing_info:"摸牌阶段开始时，你可以选择放弃摸牌并进行1次判定，此回合可以将任意1张与该判定牌不同颜色的手牌当【决斗】使用。",
					jlsg_wuwei_info:'摸牌阶段，你可以放弃摸牌，改为亮出牌堆顶的3张牌，其中每有1张基本牌，你便可视为对1名其他角色使用1张杀(每阶段对每名角色限1次)。然后将这些基本牌置入弃牌堆，其余收入手牌。',
					jlsg_yansha_info:'摸牌阶段，你可以少摸1张牌。若如此做，本回合弃牌阶段开始时，你可以将1张手牌置于武将牌上，称为［掩］。当1名角色使用杀选择目标后，你可以将1张［掩］置入弃牌堆，然后获得其2张牌。',
					jlsg_yansha2_info:'当1名角色使用杀选择目标后，你可以将1张［掩］置入弃牌堆，然后获得其2张牌。',
					jlsg_zhonghou_info:'当你攻击范围内的1名角色需要使用或打出1张基本牌时，该角色可以向你请求之，然后你摸1张牌并失去1点体力，然后视为该角色使用此牌；若你拒绝，则取消此次响应。（每名角色的回合限1次）',
					jlsg_liuyun_info:'出牌阶段限1次，你可以横置你的武将牌并弃置1张黑色牌，然后令1名角色选择1项：回复1点体力，或摸2张牌。',
					jlsg_lingbo_info:'当1名其他角色回合结束时，若你的武将牌横置时，你可以将一张自己装备区的牌移至该角色的合理区域；当1名其他角色回合开始时，若你的武将牌重置时，你可以选择该角色一张除手牌的牌，将此牌置入牌顶。',
					jlsg_qingcheng_zhu_info:'游戏开始时，若你拥有技能［流云］：你可以重置你的武将牌，视为你使用或打出1张【杀】；你可以横置你的武将牌，视为你使用或打出1张【闪】；否则技能效果反之。',
					jlsg_qingcheng_yin_info:'你可以重置你的武将牌，视为你使用或打出1张【杀】；你可以横置你的武将牌，视为你使用或打出1张【闪】。',
					jlsg_qingcheng_yang_info:'你可以横置你的武将牌，视为你使用或打出1张【杀】；你可以重置你的武将牌，视为你使用或打出1张【闪】。',
					jlsg_aozhan_info:'每当你因杀或决斗造成或受到1点伤害后，你可将牌堆顶的1张牌置于你的武将牌上，称为［战］。出牌阶段限1次，你可以选择1项：1、将所有［战］收入手牌。2、将所有［战］弃置，然后摸等量的牌。',
					...
					jlsg_guicai_info:'在任意角色的判定牌生效前，你可以选择1项：1、打出1张手牌代替之。2、亮出牌堆顶的1张牌代替之。',
					jlsg_langgu_info:'每当你造成或受到1次伤害后，你可以进行1次判定，若为黑色，你获得对方1张牌。',
					jlsg_zhuizun_info:'限定技，当你进入濒死状态时，你可以恢复体力至1点，令所有其他角色依次交给你1张手牌。然后当前回合结束后，你进行1个额外的回合。',
					jlsg_tianshang_info:'限定技，你死亡时，可令1名其他角色获得你当前另外1项技能，增加1点体力上限并恢复1点体力。',
					jlsg_yiji:'每当你受到一点伤害，可以观看牌堆顶的两张牌，并将其交给任意1~2名角色',
					jlsg_old_yiji_info:'当你受到1次伤害，可以观看牌堆顶的2张牌，并将其交给任意名角色，若你将所有的牌交给了同1名角色，你进行1次判定：判定牌为红桃，恢复1点体力。',
					jlsg_huiqu_info:'回合开始阶段，你可以弃置1张手牌进行1次判定，若结果为红色，你将场上的1张牌移动到1个合理的位置；若结果为黑色，你对1名角色造成1点伤害，然后你摸1张牌。',
					jlsg_zhaoxiang_info:'当1名其他角色使用【杀】指定目标后，若其在你攻击范围内，你令其选择1项：1、获得你的1张牌。2、令此【杀】无效；若该角色不在你攻击范围内，你可以弃置1张牌令其做上述选择。',
					jlsg_zhishi_info:'出牌阶段限一次，你可以令一名其他角色选择一项：1、弃置一张基本牌，然后回复一点。2、受到你造成的一点伤害，然后回复一点体力。',
					jlsg_old_zhishi_info:'出牌阶段限1次，你可以指定1名有手牌的其他角色，你选择其中1项执行：1.你展示1张【杀】令其弃置1张【杀】，若其执行，你与其恢复1点体力，否则你对其造成1点伤害；2.你展示1张【闪】令其弃置1张【闪】，若其执行，你与其恢复1点体力，否则你对其造成1点伤害。',
					jlsg_jianxiong_info:'主公技。每当其他魏势力受到不为你的1次伤害后，该角色可以弃置1张手牌，然后令你获得对其造成伤害的牌。',
					jlsg_jiuzhu_info:'每当1张非转化的【闪】进入弃牌堆时，你可以用1张不为【闪】的牌替换之。若此时不是你的回合，你可以视为对当前回合角色使用1张无视防具的【杀】。',
					jlsg_tuwei_info:'每当1张非转化的【杀】进入弃牌堆时，若你是此【杀】的目标或使用者，你可以弃置1张能造成伤害的牌，然后弃置此牌目标或使用者的共计2张牌。',
					jlsg_xujin_info:'摸牌阶段，你可以放弃摸牌，改为展示牌堆顶的5张牌，并令1名角色获得其中1种花色的所有牌，再将其余的牌置入弃牌堆。若如此做，你本回合的攻击范围和可以使用的【杀】数量与以此法被获得的牌的数量相同。',
					jlsg_paoxiao_info:'出牌阶段，当你使用【杀】对目标角色造成1次伤害并结算完毕后，你可以摸1张牌，然后选择1项：使用1张无视距离的【杀】，或令该角色弃置你的1张牌。',
					jlsg_benxi_info:'锁定技，你计算与其他角色的距离时始终-1.你使用【杀】指定目标后，目标角色须弃置1张装备牌，否则此【杀】不可被【闪】响应。',
					jlsg_yaozhan_info:'出牌阶段限1次，你可以与1名其他角色拼点：若你赢，视为对其使用1张【杀】并摸1张牌（此【杀】不计入每回合的使用限制）；若你没赢，该角色可以对你使用1张【杀】。',
					jlsg_wenjiu_info:'出牌阶段限1次，你可以将1张黑色手牌置于你的武将牌上，称为［酒］。当你使用【杀】选择目标后，你可以将1张［酒］置入弃牌堆，然后当此【杀】造成伤害时，该伤害+1；当此【杀】被【闪】响应后，你摸1张牌。',
					jlsg_shuixi_info:'回合开始阶段开始时，你可以展示1张手牌并选择1名其他角色，令其选择1项：弃置1张与之相同花色的手牌，或失去1点体力。若该角色因此法失去体力，则此回合的出牌阶段，你不能使用【杀】。',
					jlsg_sanfen_info:'出牌阶段限1次，你可以选择2名其他角色，其中1名你选择的角色须对另1名角色使用1张【杀】，然后另1名角色须对你使用1张【杀】，你弃置不如此做者1张牌。（使用杀有距离限制）',
					jlsg_guanxing_info:'回合开始/结束阶段开始时，你可以观看牌堆顶的Ｘ张牌（Ｘ为存活角色的数量，且最多为3），将其中任意数量的牌以任意顺序置于牌堆顶，其余以任意顺序置于牌堆底。',
					jlsg_weiwo_info:'锁定技，当你有手牌时，你防止受到的属性伤害；当你没有手牌时，你防止受到的非属性伤害。',
					jlsg_shouji_info:'出牌阶段限1次，你可以弃置1张牌并选择2名角色，然后根据你弃置牌的花色，视为其中1名你选择的角色对另1名角色使用1张牌：黑桃【决斗】，梅花【过河拆桥】，红桃【顺手牵羊】，方片【火攻】。',
					jlsg_hemou_info:'其他角色的出牌阶段开始时，你可以将1张手牌正面朝上交给该角色，该角色本阶段限1次，可将1张与之相同花色的手牌按下列规则使用：黑桃【决斗】，梅花【借刀杀人】，红桃【顺手牵羊】，方片【火攻】。',
					jlsg_qicai_info:'每当你失去1次手牌时，你可以进行判定，若结果为红色，你摸1张牌。',
					jlsg_rende_info:'任1角色的回合结束阶段结束时，你可以将任意数量的手牌交给该角色，然后该角色进行1个额外的出牌阶段。',
					jlsg_chouxi_info:'出牌阶段限1次，你可以弃置1张手牌并展示牌堆顶的2张牌，然后令1名其他角色选择1项：弃置1张与之均不同类别的牌，然后令你获得这些牌；或受到你造成的1点伤害并获得其中1张牌，然后你获得其余的牌。',
					jlsg_yongbing_info:'主公技，当1名其他蜀势力角色使用【杀】造成1次伤害后，该角色可令你摸1张牌。',
					jlsg_yinmeng_info:'出牌阶段限X次，若你有手牌，你可以展示1名其他男性角色的1张手牌，然后展示你的1张手牌，若2张牌类型相同，你与其各摸1张牌；若不同，你弃置其展示的牌，X为你所损失的体力且至少为1',
					jlsg_xiwu_info:'当你使用的【杀】被目标角色的【闪】响应后，你可以摸1张牌，然后弃置其1张手牌。',
					jlsg_juelie_info:'出牌阶段限1次，你可以令1名手牌数与你不同的其他角色选择1项：将手牌数调整至与你相等；或视为你对其使用1张【杀】（不计入出牌阶段的使用限制）。',
					jlsg_xianger_info:"1名其他男性角色的回合开始时，你可以交给其2张基本牌。若如此做，该角色跳过出牌阶段，然后可以视为对你使用1张【杀】，否则下回合的出牌阶段受到你的1点伤害；若其在此阶段未造成伤害，则跳过弃牌阶段，且你摸1张牌。",
					jlsg_fangxin_info:'当你需要使用1张【桃】时，你可以将1张梅花牌当【兵粮寸断】或将1张方片牌当【乐不思蜀】对自己使用，若如此做，视为你使用1张【桃】。',
					jlsg_xiyu_info:'你的回合开始时，你可以弃置1名角色的1张牌，然后该角色进行1个额外的出牌阶段。',
					jlsg_wanrou_info:'你的方片牌或你判定区的牌进入弃牌堆时，你可以令1名角色摸1张牌。',
					jlsg_zhouyan_info:'出牌阶段，你可以令1名其它角色摸1张牌，若如此做，视为你对其使用1张【火攻】，你可以重复此流程直到你以此法未造成伤害。每当你使用【火攻】造成1次伤害后，你可以摸1张牌。',
					jlsg_zhaxiang_info:'出牌阶段，你可以将一张手牌扣置，然后令一名其它角色选择一项：交给你一张牌并弃置你扣置的牌；或展示你扣置的牌并获得之。若你扣置的牌为【杀】，则视为你对其使用一张火属性的【杀】（不计入出牌阶段的使用限制）。',
					jlsg_old_zhaxiang_info:'出牌阶段限1次，你可以指定1名其它角色，视为该角色对你使用1张【杀】，然后你摸2张牌并视为对其使用1张【杀】（你的此【杀】无视防具）。',
					jlsg_shixue_info:'当你使用【杀】指定目标后，你可以摸2张牌，当此【杀】被【闪】响应后，你须弃置1张牌。',
					jlsg_guoshi_info:'任1角色的回合开始阶段开始时，你可以观看牌堆顶的2张牌，然后可将其中任意张牌置于牌堆底；任1角色的回合结束阶段开始时，你可以令其获得本回合因弃置或判定进入弃牌堆的1张牌。',
					jlsg_yingcai_info:'摸牌阶段，你可以放弃摸牌，改为展示牌堆顶的1张牌，你重复此流程直到你展示出第3种花色的牌时，将这张牌置入弃牌堆，然后获得其余的牌。',
					jlsg_old_yingcai_info:'摸牌阶段，你可以放弃摸牌，改为展示牌堆顶的一张牌，你重复此流程直到你展示出第三种花色的牌时，将这张牌置入弃牌堆，然后获得其余的牌。',
					jlsg_weibao_info:'出牌阶段限1次，你可以将1张手牌置于牌堆顶，然后令1名其他角色选择1种花色后摸1张牌并展示之，若此牌与所选花色不同，你对其造成1点伤害。',
					jlsg_choulve_info:'出牌阶段限1次，你可以交给2名其他角色各1张手牌，然后依次展示之，若点数不同，视为点数较大的1方对另1方使用1张【杀】，该【杀】造成伤害后，你摸1张牌。',
					jlsg_jiexi_info:'出牌阶段，你可以与一名其他角色拼点，若你赢，视为对其使用一张【过河拆桥】。你可重复此流程直到你以此法拼点没赢。',
					jlsg_old_jiexi_info:'若你的武将牌正面朝上，你可以指定1名有手牌的角色进行拼点：若你赢，你视为对其使用1张【过河拆桥】，否则本回合不可发动此技能；锁定技，若你的武将牌正面朝上并触发技能〈劫袭〉后，且你的手牌数小于4时，你将武将牌背面朝上并摸1张牌；若你的武将牌背面朝上，你不能成为【南蛮入侵】和【闪电】的目标。',
					jlsg_youxia_info:'出牌阶段，若你的武将牌正面朝上，你可以将你的武将牌翻面，然后从一至两名其他角色处各获得一张牌；锁定技，若你的武将牌背面朝上，你不能成为【杀】和【决斗】的目标。',
					jlsg_old_youxia_info:'出牌阶段限1次，你可以将你的武将牌翻面，然后从1至2名其他角色的区域各弃置1张牌；锁定技，若你的武将牌背面朝上，你不能成为【杀】和【兵粮寸断】的目标。',
					
					
					jlsg_huailing_info:'若你的武将牌背面朝上，其他角色使用1张锦囊牌指定大于1个目标时，你可以令1名其他角色不受到该牌的效果，然后你将武将牌正面朝上；锁定技，若你的武将牌背面朝上，你不能成为【决斗】和【过河拆桥】的目标。',
					jlsg_dailao_info:'你可以将武将牌背面朝上并摸1张牌，视为对1名其他角色使用1张【杀】，然后弃置该目标的1张牌。',
					jlsg_youdi_info:'若你的武将牌背面朝上，你可以将其翻面来视为你使用1张【闪】。每当你使用【闪】响应1名角色使用的【杀】时，你可以额外弃置1-X张牌，然后该角色弃置等量的牌（X为该角色的除判定区的牌数）。',
					jlsg_ruya_info:'当你失去最后的手牌时，你可以翻面并将手牌补至你体力上限的张数。',
					jlsg_quanheng_info:'出牌阶段限1次，你可以将至少1张手牌当1张【无中生有】或【杀】使用，若你以此法使用的牌被【无懈可击】或【闪】响应时，你摸等量的牌。',
					jlsg_xionglve_info:'摸牌阶段，你可以放弃摸牌，改为展示牌堆顶的2张牌，你获得其中1张牌，然后将另1张牌置于你的武将牌上，称为［略］。出牌阶段，你可以将1张基本牌或锦囊牌的［略］当与之同类别的任意1张牌（延时类锦囊牌除外）使用，将1张装备牌的［略］置于1名其他角色装备区内的相应位置。',
					jlsg_fuzheng_info:'主公技，回合开始阶段开始时，你可以令至多2名其他吴势力角色各摸1张牌，然后这些角色依次将1张手牌置于牌堆顶。',
					jlsg_jiwu_info:'出牌阶段限1次，你可以将你的手牌调整至1张，若如此做，本回合你的攻击范围无限，且你下1次使用的【杀】造成的伤害+1。锁定技，若你的装备区没有牌，你使用【杀】可以至多额外指定任意2名其他角色为目标。',
					jlsg_old_jiwu_info:'出牌阶段限1次，若你的手牌数大于1，若如此做，本回合你的攻击范围无限，且你下1次使用的【杀】造成的伤害+1。锁定技，若你的装备区没有牌，你使用【杀】可以至多额外指定任意2名其他角色为目标。',
					
					jlsg_sheji_info:'当1名装备区有武器牌的其他角色对另1名角色造成伤害后，你可以弃置1张牌，然后获得该角色的武器牌。你可以将装备牌当无距离限制的【杀】使用或打出，你以此法使用的【杀】须连续使用2张【闪】才能抵消。',
					jlsg_xingyi_info:'出牌阶段限1次，你可以获得1名有手牌的其他角色1张手牌，然后令其恢复1点体力。',
					jlsg_guagu_info:'当1名角色进入濒死状态时，你可以弃置其所有手牌（至少1张），然后该角色恢复1点体力。若你以此法弃置其2张或更多的手牌时，该角色摸1张牌。',
					jlsg_wuqin_info:'回合结束阶段结束时，你可以弃置1张基本牌，然后选择1项：摸2张牌，或进行1个额外的出牌阶段。',
					jlsg_lijian_info:'出牌阶段限1次，你可以弃1张牌并选择2名其他男性角色。若如此做，视为其中1名男性角色对另1名男性角色使用1张【决斗】(该【决斗】不可被【无懈可击】响应)。',
					jlsg_manwu_info:'出牌阶段限1次，你可以展示1名男性角色的1张手牌，若此牌为方片，将之置于该角色的判定区内，视为【乐不思蜀】；若不为方片，你获得之。',
					jlsg_baiyue_info:'锁定技，回合结束阶段开始时，你获得本回合的角色进入弃牌堆的1张牌。',
					jlsg_ganglie_info:'出牌阶段开始时，你可以失去1点体力，若如此做，你本回合下1次造成的伤害+1。且本回合你每造成1点伤害，回合结束时你便摸1张牌',  
					},
                };
				if(jilvesanguo.forbidBtCharacter){
					var list=[
					...
					for(var i=0;i<list.length;i++){
						delete jlsg_sr.character[i];
					}
				}
				if(jilvesanguo.oldCharacterReplace){
                ...
                if(lib.device||lib.node){
                    for(var i in jlsg_sr.character){jlsg_sr.character[i][4].push('ext:民间极略/'+i+'.jpg');}
                }else{
                    for(var i in jlsg_sr.character){jlsg_sr.character[i][4].push('db:extension-民间极略:'+i+'.jpg');}
                }
                return jlsg_sr;
            });	
			
            lib.config.all.characters.push('jlsg_sr');
            ...
            game.import('character',function(){
                var jlsg_soul = {
                    ...
                    connect:true,	
                    character:{ 
						jlsgsoul_zuoci:['male','qun',3,['jlsg_qimen','jlsg_jinji','jlsg_xuhuan'],['die_audio']],
						jlsgsoul_caocao:['male','wei',3,['jlsg_guixin','jlsg_feiying_new'],['zhu','die_audio']],
						jlsgsoul_sunquan:['male','wu',4,['jlsg_huju'],['zhu','die_audio']],
						jlsgsoul_jiaxu:['male','wei',3,['jlsg_yanmie','jlsg_shunshi'],['die_audio']],
						jlsgsoul_liubei:['male','shu',4,['jlsg_junwang','jlsg_jizhao'],['zhu','die_audio']],
						jlsgsoul_zhugeliang:['male','shu',3,['jlsg_xingchen','jlsg_kuangfeng','jlsg_dawu'],['die_audio']],
						jlsgsoul_simayi:['male','wei',3,['jlsg_jilve','jlsg_tongtian'],['nei','die_audio']],
						jlsgsoul_luxun:['male','wu',3,['jlsg_jieyan','jlsg_fenying'],['die_audio']],
						jlsgsoul_lvbu:['male','qun',5,['jlsg_kuangbao','jlsg_wumou','jlsg_wuqian','jlsg_shenfen'],['die_audio']],
						jlsgsoul_guanyu:['male','shu',5,['jlsg_wushen','jlsg_suohun'],['die_audio']],
						jlsgsoul_zhaoyun:['male','shu',2,['jlsg_juejing','jlsg_longhun'],['die_audio']],
						jlsgsoul_zhangliao:['male','wei',5,['jlsg_nizhan','jlsg_cuifeng','jlsg_weizhen'],['die_audio']],
						jlsgsoul_huangyueying:['female','shu',3,['jlsg_zhiming','jlsg_suyin'],['die_audio']],
						jlsgsoul_zhangjiao:['male','qun',3,['jlsg_dianjie','jlsg_shendao','jlsg_leihun'],['fan','die_audio']],
						jlsgsoul_lvmeng:['male','wu',3,['jlsg_shelie','jlsg_gongxin'],['die_audio']],
						jlsgsoul_guojia:['male','wei',3,['jlsg_tianqi','jlsg_tianji'],['zhu','die_audio']],
						jlsgsoul_diaochan:['female','qun',3,['jlsg_tianzi','jlsg_meixin'],['die_audio']],
						jlsgsoul_zhangfei:['male','shu',4,['jlsg_shayi','jlsg_zhenhun'],['die_audio']],
						jlsgsoul_simahui:['male','qun',3,['jlsg_zhitian','jlsg_yinshi'],['zhu','die_audio']],
						jlsgsoul_sunshangxiang:['female','shu',3,['jlsg_xianzhu','jlsg_liangyuan'],['zhong','die_audio']],
						jlsgsoul_ganning:['male','wu',4,['jlsg_lvezhen','jlsg_youlong'],['die_audio']],
						jlsgsoul_xiahoudun:['male','wei',4,['jlsg_danqing','jlsg_zhonghun'],['die_audio']],
						jlsgsoul_dianwei:['male','wei',6,['jlsg_zhiji'],['die_audio']],
						jlsgsoul_huatuo:['male','qun',3,['jlsg_jishi','jlsg_xuanxin'],['zhong','die_audio']],
						jlsgsoul_zhouyu:['male','wu',4,['jlsg_qinyin','jlsg_yeyan'],['die_audio']],	
					},
					characterIntro:{
   ...
   game.addVideo('storage',player,['jlsg_qimen',player.storage.jlsg_qimen]);				     
    ...
    jlsg_qimen2:{
                audio:"ext:民间极略:true",
                trigger:{player:["changeHp","enterGame"],
                global:'gameDrawBegin'
                },
                forced:true, 
                unique:true,
         ...
         game.log(player,'失去了技能','【奇门】');
			...
			jlsg_dunjia:{
				...
				marktext:"遁",
				intro:{
					content:"限定技，你的回合开始时，可以选择1名无［禁］的其他角色，该角色获得技能［奇门·改］",
				},
				content:function (){
					'step 0'
 ...
 jlsg_guixin1:{
					...
					jlsg_huju2:{
						...
						content:function(){
							...
							if(result.control=='选项一'){
								...
								lib.skill.zhiheng.audioname=['jlsgsoul_sunquan'];
			...
			jlsg_jizhao:{
                ...
                ai:{
					...
					jlsg_xingchen:{
						...
						marktext:'星',
						filter:function(event,player){
							return !player.storage.jlsg_xingchen;
						},
						content:function(){
							"step 0"
							...
							player.storage.jlsg_xingchen=result.cards;
							player.syncStorage('jlsg_xingchen');
						},
						mark:true,
						intro:{
							mark:function(dialog,content,player){
					...
					jlsg_xingchen2:{
						...
						content:function(){
							"step 0"
							player.chooseCard(get.prompt('jlsg_xingchen'),[1,3]).ai=function(card){
								return 1;
							};
							"step 1"
							if(result.bool){
								player.logSkill('jlsg_xingchen');
								player.lose(result.cards,ui.special)._triggered=null;
								player.storage.jlsg_xingchen=player.storage.jlsg_xingchen.concat(result.cards);
								player.syncStorage('jlsg_xingchen');
							...
							"step 2"
							player.chooseCardButton(player.storage.jlsg_xingchen,'选择1-2张牌作为手牌',[1,2],true).ai=function(button){
						...
						player.gain(result.links)._triggered=null;
							for(var i=0;i<result.links.length;i++){
								player.storage.jlsg_xingchen.remove(result.links[i]);
							}
							player.syncStorage('jlsg_xingchen');
							if(player==game.me&&_status.auto){
								game.delay(0.5);
					...
					jlsg_kuangfeng:{
						...
						content:function(){
							"step 0"
							var clearKuangfeng=sgs.findPlayerBySkillName('jlsg_kuangfeng2');
							if(clearKuangfeng){
							...
							player.chooseTarget('选择1名角色获得狂风标记').ai=function(target){
								if(player.storage.jlsg_xingchen.length>3) return sgs.isWeak(target)&&sgs.isEnemy(player,target);
								return -1;
							}
							"step 1"
							if(result.bool){
								...
								player.chooseCardButton('弃置1枚［星］',player.storage.jlsg_xingchen,true);
							}
							else{
								event.finish();
							}
							"step 2"
							player.storage.jlsg_xingchen.remove(result.links[0]);
							player.syncStorage('jlsg_xingchen');
							if(player.storage.jlsg_xingchen.length==0){
								player.unmarkSkill('jlsg_xingchen');
							}
					...
					jlsg_kuangfeng2:{
						...
						forced:true,
						content:function(){
							var jlsg_zhugeliang=sgs.findPlayerBySkillName('jlsg_kuangfeng');
							if(jlsg_zhugeliang){
								if(trigger.nature){
									...
									if(jlsg_zhugeliang&&jlsg_zhugeliang.storage.jlsg_xingchen){
										jlsg_zhugeliang.line(player,'water');
										var card=get.cards();
										jlsg_zhugeliang.$draw(1);
										jlsg_zhugeliang.lose(card,ui.special)._triggered=null;
										jlsg_zhugeliang.storage.jlsg_xingchen=jlsg_zhugeliang.storage.jlsg_xingchen.concat(card);
										jlsg_zhugeliang.markSkill('jlsg_xingchen');
										jlsg_zhugeliang.syncStorage('jlsg_xingchen');
										game.log(jlsg_zhugeliang,'将牌堆顶得1张牌置入［星］');
									}
								}
							}
						},
					...
					jlsg_dawu:{
						...
						content:function(){
							...
							[1,Math.min(game.players.length,player.storage.jlsg_xingchen.length)]).ai=function(target){
								...
								if(att>=4){
									...
									if(sgs.isWeak(target)) return att*1.1;
									return 0;
								}
								return -1;
							}
							...
							if(result.bool){
								...
								player.chooseCardButton('弃置'+get.cnNumber(length)+'枚［星］',length,player.storage.jlsg_xingchen,true);
							}
							else{
								event.finish();
							}
					...
					//	},
					  intro:{
                       content:true,
						...
						content:function(){
							"step 0"
							//player.storage.jlsg_tongtian=true;
							for(var i=0;i<cards.length;i++){
								if(get.suit(cards[i])=='heart'){//红心
									player.addSkill('jlsg_tongtian_shu');
								
								}
								if(get.suit(cards[i])=='diamond'){//方块
									player.addSkill('jlsg_tongtian_qun');
									//lib.skill.jlsg_quanheng.audio=['jlsgsoul_simayi'];
								}
								if(get.suit(cards[i])=='club'){//梅花
									player.addSkill('jlsg_tongtian_wei');
									//lib.skill.jlsg_lingxin.audio=['jlsgsoul_simayi'];
								}
								if(get.suit(cards[i])=='spade'){//黑桃
									player.addSkill('jlsg_tongtian_wu');
									//lib.skill.jlsg_shichou.audio=['jlsgsoul_simayi'];
								}
							}
							"step 1"
    ...
    content:function (){
        player.gainPlayerCard(get.prompt('jlsg_tongtian_wei',trigger.source),trigger.source,get.buttonValue,'he').set('logSkill',['jlsg_tongtian_wei',trigger.source]);
    },
    ai:{
        "maixie_defend":true,
        effect:{
            target:function (card,player,target){
				...
				jlsg_tongtian_shu:{
					...
					jlsg_fenying:{
						...
						content:function (){
							...
							player.chooseCardTarget({
								...
								position:'he',
								prompt:'焚营：弃置1张红色牌对目标或与其相距最近的其他目标造成等量火焰伤害'
							});
							"step 1"
							if(result.bool){
					...
					jlsg_zhiming:{
						...
						content:function(){
							"step 0"
							player.chooseToDiscard('h','知命:你可以弃置1张手牌，然后弃置其1张手牌，若2张牌颜色相同，你令其跳过此回合的摸牌阶段或出牌阶段').ai=function(card){
								if(get.attitude(player,trigger.player)<0)
									return 10-get.value(card);
								return 0;
							}
							...
							if(result.control=='跳过出牌'){
								...
								trigger.player.addTempSkill('jlsg_zhiming_buff',{player:'phaseEnd'}); 
							}
						},
						ai:{
							expose:0.4
					...
					jlsg_suyin:{
						...
						content:function(){
							"step 0"
							player.chooseTarget('【夙隐】：选择1名角色将其翻面',function(card,player,target){
								return player!=target;
							}).ai=function(target){
					...
					jlsg_dianjie:{
						...
						group:"jlsg_dianjie2",
						filter:function (event,player) {
							return event.card.name=='shan'&&_status.currentPhase!=player&&!player.storage.jlsg_shendao;
						},
						content:function () {
							player.useSkill('jlsg_dianjie_buff');
						},
						ai:{
							useShan:true,
							effect:{
						...
						content:function () {
							...
							if(result.judge>0){
								player.chooseTarget('选择1个目标对其造成1点雷电伤害').ai = function (target) {
									if(player.hp<=1&&player.hasSkill('jlsg_leihun')) return target==player?9:-1;
									return get.damageEffect(target, player, player, "thunder");
								}
							}
							else{
					...
					jlsg_shendao:{
						...
						content:function (){
							...
							"step 2"
							if(result.bool&&result.links){
								player.storage.jlsg_shendao=true;
								event.cards.remove(result.links[0]);
							...
							"step 4"
							delete player.storage.jlsg_shendao;
					...
					jlsg_tianqi:{
						...
						chooseButton:{
							...
							prompt:function(links,player){
								return '亮出牌堆顶的1张牌，并将此牌当'+get.translation(links[0][2])+'使用。若亮出的牌不为'+get.translation(links[0][0])+'牌，你须先失去1点体力。(你的出牌阶段限1次。)';
							}
						},
						group:['jlsg_tianqi2','jlsg_tianqi3','jlsg_tianqi4','jlsg_tianqi5'],
						ai:{
							order: 10,
							result:{
								player: function (player) {
            ...
            jlsg_tianqi2: {
                ...
                check: function (card, event, player) {
                    var player = _status.event.player;
                    var cards = get.cards();
                    if (player.hp > 1&&get.type(cards[0]) == 'basic') {
                        return 1;
                    }
                    if (player.hp == 1 && player.countCards('h', 'tao')) return 0;
                    return 1;
                },
                viewAs: { name: 'tao' },
                viewAsFilter: function (player) {
                ...
                ai: {
                    skillTagFilter: function (player) {
                        return player.storage.jlsg_tianqi == 'basic' && _status.dying != player;
                    },
                    threaten: 1.5,
                    save: true,
                    respondTao: true,
            ...
            jlsg_tianqi3: {
                ...
                check: function (card, event, player) {
                    ...
                    if (player.hp <=2 && player.countCards('h', 'sha') && player.storage.sgk_tianji_top != 'basic') return 0;
                    return 1;
                },
                viewAs: { name: 'sha' },
                viewAsFilter: function (player) {
                ...
                ai: {
                    ...
                    basic: {
                        order: function (card, player) {
                            if (player.hasSkillTag('presha')) return 5;
                            return 2;
                        },
                        useful: [5, 1],
                        value: [5, 1],
            ...
            jlsg_tianqi4: {
                ...
                ai2: function (target) {
                    var player = _status.event.player;
                    var cards = get.cards();
                    if (player.hp >2&&get.type(cards[0]) == 'trick') {
                        return 1;
                    }
                    ...
                    if (player.countCards('h', 'wuxie') && player.hp <= 2 && player.storage.jlsg_tianqi != 'trick') return 0;
                    return 0;
                },
                selectCard: -1,
                viewAs: { name: 'wuxie' },
                ...
                ai: {
                    skillTagFilter: function (player) {
                        return player.storage.jlsg_tianqi == 'trick' && _status.dying != player;
                    },
                    basic: {
                        useful: [6, 4],
                        value: [6, 4],
            ...
            jlsg_tianqi5: {
                ...
                check: function (card, event, player) {
                    ...
                    if (player.hp <=2 && player.countCards('h', 'shan') && player.storage.sgk_tianji_top != 'basic') return 0;
                    return 1;
                },
                viewAs: { name: 'shan' },
                viewAsFilter: function (player) {
                ...
                ai: {
					...
					jlsg_tianji:{
						...
						content:function(){
							...
							if(result.control=='获得'){
								...
								player.chooseCard('选择1张牌置于牌堆顶','h',true).ai=function(card){
									if(_status.currentPhase==player){
							...
							'step 2'
							event.card=result.cards[0];
							player.storage.jlsg_tianji_top=get.type(event.card,'trick');
					...
					jlsg_tianzi:{
						...
						content:function(){
					       ...
					       trigger.cancel();
							event.current=player.next;
							"step 1"
							event.current.chooseToDiscard("he",2,'弃置2张牌或令其摸1张牌').ai=function(card){
					...
					jlsg_meixin:{
						...
						content:function(){
							target.markSkillCharacter('jlsg_meixin',player,'魅心','本阶段当你使用1张基本牌后，该目标弃置1张牌；当你使用1张锦囊牌后，你获得该目标1张牌；当你使用1张装备牌后，你对该目标造成1点伤害。');
		...
		ai:{
					...
					jlsg_zhitian:{
						...
						content:function(){
							"step 0"
							player.chooseTarget('知天：将所有手牌交给1名角色',true).ai=function(target){
								return get.attitude(player,target);
							}
							"step 1"
							if(result.bool){
				...
				var players=game.players.concat(game.dead);
					...
					jlsg_xuanxin:{
                        ...
                        marktext:"心",
                        init:function (player){
        player.storage.jlsg_xuanxin=3;
        player.markSkill('jlsg_xuanxin');
        player.syncStorage('jlsg_xuanxin');
    },
                        intro:{
    mark:function(dialog,content,player){
       var num=0;
      ...
      dialog.add("你的悬心："+player.storage.jlsg_xuanxin+"次/3次<br><br>"+"弃牌堆红桃牌"+num+"张");
         }
     },          
						filter:function(event,player){
						var cards=[];
						...
						content:function(){
							...
							if(cards.length){
								...
								if(trigger.player&&trigger.player.hp>0&&trigger.player!=player){
									player.chooseCard('是否交给'+get.translation(trigger.player)+'1张牌？').ai=function(card){
    	...
    	filter:function(card,player,target){
						...
						content:function(){
							...
							player.chooseControl('摸'+player.hp+'张牌','弃'+player.hp+'张牌').ai=function(){
								if(get.attitude(player,target)>0) return '摸'+player.hp+'张牌';
								return '弃'+player.hp+'张牌';
							};
							'step 1'
							if(result.control=='摸'+player.hp+'张牌'){
							    player.loseHp();
								target.draw(player.hp);
								//player.draw(Math.ceil((player.maxHp-player.hp)/2));
							}
							else{
							    player.recover();
								target.chooseToDiscard(player.hp,'he',true);
								//player.draw(Math.ceil((player.maxHp-player.hp)/2));
							}	
						},
						ai:{
		...
		                    if (event.list[result.index] == '摸2张牌，然后令所有角色各失去1点体力。') {
								player.draw(2);
								var players = game.filterPlayer();
								player.logSkill('jlsg_qinyin', players);
								for (var i = 0; i < players.length; i++) {
							    	players[i].loseHp();
							    	game.delay();
								}
		...
		            },
					jlsg_yeyan:{
						...
						content:function (){
							...
							if(cards.length>=3){
							player.loseHp(3);}
							if(targets.length<2){target.damage('fire',cards.length);}
							else{targets[0].damage('fire',cards.length);targets[1].damage('fire',cards.length);}
						},
						intro:{
							content:'limited'
						},
					...
					translate:{ 
						...
						jlsg_dawu2:'大雾',
						jlsg_tongtian:'通天',
						jlsg_tongtian_shu:'星占·蜀技',
						jlsg_tongtian_wei:'神算·魏技',
						jlsg_tongtian_wu:'制衡·吴技',
						jlsg_tongtian_qun:'刻死·群技',
						jlsg_tongtian_wei_info:"当你受到伤害后，你可以获得伤害来源的1张牌。",
						jlsg_tongtian_shu_info:"准备阶段，你可以观看牌堆顶的5张牌，并将其以任意顺序置于牌堆项或牌堆底。",
						jlsg_tongtian_qun_info:"锁定技，你的回合内，除你以外，不处于濒死状态的角色不能使用【桃】。",
						jlsg_tongtian_wu_info:"出牌阶段1次，你可以弃置任意张牌，然后摸等量的牌。",
						jlsg_jilve:'极略',
						jlsg_jieyan:'劫焰',
						...
						jlsg_feiying_new_info:'锁定技，若你的武将牌正面朝下，你不能成为【杀】的目标。',
						jlsg_guixin_info:'<li>若在场的游戏玩家〈游戏角色〉大于4时，你受到1次伤害后，你可以随机获得每名其他角色区域里的1张牌，再摸X张牌（X为阵亡的角色数，最多为3人），然后翻面。<li>若在场的游戏玩家〈游戏角色〉不大于5时，你受到1次伤害后，你可以随机获得每名其他角色区域里的1张牌，再摸X张牌（X为你的已损失体力）。',
						jlsg_qinyin_info:'弃牌阶段开始时，你可以选择1项：1.摸2张牌，然后令所有角色各失去1点体力；2.弃2张牌，然后令所有角色各恢复1点体力。',
						jlsg_yeyan_info:'限定技，出牌阶段，你可以弃置至少1种花色不同的手牌，然后对1至2名角色各造成等量的火属性伤害，若你以此法弃置的手牌花色数不少于3，你将失去3点体力。',
						jlsg_huju_info:'锁定技，其他角色的回合开始时，你摸1张牌。你的回合开始时，若你的手牌数为最多（或之1），你选择1项：1、失去1点体力；2、减1点体力上限，失去〈虎踞〉，获得〈制衡〉和〈虎缚〉。',
						jlsg_hufu_info:'出牌阶段限1次，你可以令1名其他角色弃置X张牌（X为其装备区的牌数）。',
						jlsg_yanmie_info:'出牌阶段，你可以弃置1张黑桃牌，令1名其他角色先弃置所有手牌再摸等量的牌并展示之，然后你可以弃置其中所有非基本牌，并对其造成等量的伤害。',
						jlsg_shunshi_info:'当你成为其他角色使用【杀】、【桃】的目标后，你可以令除该角色以外的1-3名其他角色各摸1张牌，然后这些角色也成为此牌的目标。',
						jlsg_junwang_info:'锁定技，其他角色的出牌阶段开始时，若其手牌数不小于你，其须交给你1张手牌。',
						jlsg_jizhao_info:'出牌阶段对1名无标记的其他角色限1次，你可以交给其至少1张手牌，并令其获得1个［诏］标记；拥有［诏］标记的角色回合结束时，若其本回合内未造成过伤害，其受到你造成的1点伤害并失去［诏］标记。',
						jlsg_xingchen_info:'分发起始手牌时，你将获得起始手牌改为观看牌堆顶11张牌并获得其中4张手牌，然后将其余7张牌扣置于武将牌上，称为［星］；摸牌阶段结束时，你可以用1-3张手牌来替换1-2枚［星］',
						jlsg_kuangfeng_info:'回合开始阶段开始时，你可以将1张［星］置入弃牌堆，然后选择1名角色获得1枚［风］标记，若如此做，当其于你的下回合开始前受到火焰伤害时，该伤害+1；雷电伤害时，你令其弃置2张牌；普通伤害时，你摸1张牌置入［星］。',
						jlsg_dawu_info:'回合结束阶段开始时，你可以弃掉至少1张［星］，然后选择等量的角色获得［雾］标记，若如此做，当其于你的下回合开始前受到非雷电伤害时，你防止之。',
						jlsg_tongtian_info:'限定技，出牌阶段你可以弃置任意花色不同的牌，然后根据以下技能获得相应技能：黑桃·吴技；红桃·蜀技；梅花·魏技；方片·群技。',
						jlsg_jilve_info:'出牌阶段，你可以摸1张牌，然后选择1项：使用1张牌，或弃置1张牌。若你以此法弃置牌，则本回合此技能失效。',
						jlsg_jieyan_info:'当1张红色【杀】或红色非延时锦囊牌仅指定1名角色为目标后，你可以弃置1张手牌令其无效，然后对目标角色造成1点火焰伤害。',
						jlsg_fenying_info:'当1名角色受到火焰伤害后，若你的手牌数不大于体力上限，你可以弃置1张红色牌，然后对该角色或与其距离最近的1名角色（不为你）造成等量的火焰伤害。',
						jlsg_kuangbao_info:'锁定技，游戏开始时，你获得2枚［暴］标记。每当你造成或受到伤害时，你获得等量的［暴］标记。',
						jlsg_wumou_info:'锁定技，当你使用非延时锦囊牌时，你须选择1项：1，弃置1枚［暴］标记；2，受到1点伤害并增加1枚［暴］标记。',
						jlsg_wuqian_info:'出牌阶段：你可以弃置2枚［暴］标记，若如此做，本回合内你视为拥有技能〈无双〉且你造成伤害后额外获得1枚［暴］标记。',
						jlsg_shenfen_info:'出牌阶段，弃6个暴怒标记，你对每名其他角色各造成1点伤害，其他角色先弃掉各自装备区里所有的牌，再各弃4张手牌，然后将你的武将牌翻面，每回合限1次。',
						jlsg_wushen_info:'锁定技，你的【杀】和【桃】均视为【决斗】。',
						jlsg_suohun_info:'锁定技，每当你受到1点伤害时，伤害来源(除你以外)获得1个［魂］标记。当你进入濒死状态时，减1半(向上取整)的体力上限并恢复体力至体力上限，拥有［魂］标记的角色依次弃置所有的［魂］标记，然后受到与弃置的［魂］标记数量相同的伤害。',
						jlsg_juejing_info:'锁定技，1名角色的回合开始时，若你的体力值：为1，你摸1张牌；大于1，你失去1点体力，然后摸2张牌。',
						jlsg_longhun_info:'你可以将同花色的X张牌按下列规则使用（或打出）：红桃当【桃】；方块当【杀】；梅花当【闪】；黑桃当【无懈可击】。（X为你当前的体力值且至少为1）。',
						jlsg_nizhan_info:'每当1名角色受到【杀】或【决斗】造成的1次伤害后，你可以将1枚［袭］标记放置在该角色或伤害来源(不为你)的武将牌上；锁定技，你的身份为“主公”时，不增加体力上限。',
						jlsg_cuifeng_info:'锁定技，回合结束阶段，若场上的［袭］标记总数不小于4，你须依次从每名被标记的角色处获得等同于其［袭］标记数量的手牌。若该角色手牌不足，则你获得其全部手牌，然后该角色受到你对其造成的1点伤害。最后移除场上全部的［袭］标记。',
						jlsg_weizhen_info:'回合开始阶段，你可以移除场上全部的［袭］标记，然后摸等同于［袭］标记数量的牌。',
						jlsg_zhiming_info:'其他角色的回合开始阶段开始时，若其有手牌，你可以弃置1张手牌，然后弃置其1张手牌，若2张牌颜色相同，你令其跳过此回合的摸牌阶段或出牌阶段；若跳过出牌阶段，该角色的回合结束时，将弃置1张牌。',
						jlsg_suyin_info:'你的回合外，当你失去最后的手牌时，可令1名其他角色将其武将牌翻面，然后令其弃置1张牌。',
						jlsg_dianjie_info:'你的回合外，当你使用或打出一张闪后，或你主动跳过出牌阶段后：你可以进行一次判定，若为黑色，可以对1名角色造成1点雷电伤害；若为红色，可以令1-2名未横置的角色进行横置。',
						jlsg_old_dianjie_info:'你可以跳过你的摸牌阶段或出牌阶段，然后判定：若结果为黑色，你对一名角色造成1点雷电伤害；若结果为红色，你令至多两名武将牌未横置的角色将其武将牌横置。',
						jlsg_shendao_info:'锁定技，对1名角色的判定牌生效前，你亮出牌堆顶的2张牌，选择其中1张直接代替之，若不是你的回合，你将另1种牌收入手牌。',
						jlsg_old_shendao_info:'一名角色的判定牌生效前，你可以用一张手牌或场上的牌代替之',
						jlsg_leihun_info:'锁定技，你受到的雷电伤害均视为体力恢复。',
						jlsg_shelie_info:'锁定技，摸牌阶段开始时，你跳过之，改为选择指定获得某种类型的牌（最多四次），然后从牌堆随机摸取之。',
						jlsg_gongxin_info:'出牌阶段限1次，你可以观看1次任意1名角色的手牌并展示其中所有的红桃牌，然后若展示的牌数：为1，你弃置之并对其造成1点伤害；大于1，你获得其中1张红桃牌。',
						jlsg_tianqi_info:'你的濒死状态除外，每当你需要使用或打出1张基本牌或非延时锦囊牌时，你可以声明之，然后亮出牌堆顶的1张牌，并将此牌当你所述之牌使用或打出，若其与你所述之牌不为同1类别，你须先失去1点体力。（但出牌阶段仅限1次。）',
						jlsg_tianji_info:'任1角色的出牌阶段开始时，你可以观看牌堆顶的1张牌，然后你可以选择1项：用1张手牌替换之；若你的手牌数不是全场最多的(或之1)，你可以获得之。',
						jlsg_xianzhu_info:'当1名角色恢复体力后，或失去装备区里的牌后，你可以令其摸2张牌。',
						jlsg_liangyuan_info:'限定技，出牌阶段，你可以选择1名其他男性角色，则于本局游戏中，你的自然回合结束时，该角色进行1个额外的回合。',
						jlsg_tianzi_info:'摸牌阶段开始时，你可以令所有其他角色依次选择1项：1、弃置2张牌；2、令你摸1张牌。',
						jlsg_meixin_info:'出牌阶段限1次，你可以弃置1张牌并选择1名其他男性角色，若如此做，本阶段当你使用1张基本牌后，你令其弃置1张牌；当你使用1张锦囊牌后，你获得其1张牌；当你使用1张装备牌后，你对其造成1点伤害。',
						jlsg_shayi_info:'锁定技，出牌阶段开始时，你展示所有手牌，若有【杀】，你摸1张牌；若没有【杀】，你于本阶段可以将1张黑色牌当【杀】使用。你使用【杀】无距离限制、无次数限制。',
						jlsg_zhenhun_info:'出牌阶段限1次，你可以弃置1张牌令所有其他角色的非锁定技于本阶段内无效。',
						jlsg_zhitian_info:'变化技，锁定技，你的回合开始时，若你的手牌数大于1时，你须将所有手牌交给1名角色，并令该角色随机获得未加入本局游戏的武将的1个技能（觉醒技、主公技、限定技和变化技除外），然后你失去1点体力。',
						jlsg_yinshi_info:'锁定技，当你受到伤害时，你防止之，然后摸等同于此伤害值数量的牌。',
						jlsg_zhiji_info:'出牌阶段限1次，你可以弃置至少1张武器牌，然后对1名其他角色造成等同于此次弃置武器牌数点伤害。当你受到伤害后，你可以从弃牌堆或牌堆随机获得1张武器牌。',
						jlsg_jishi_info:'出牌阶段限1次，你可以弃置1张红桃牌选择1名角色，然后触发以下效果：1.若其的手牌大于等于其的体力上限时，先弃置所有手牌再摸等量的牌并展示，其中每有1张非基本牌，你令其恢复1点体力，以此法恢复的过量体力效果改为其摸等量的牌；2.若其的手牌不大于其的体力上限时，令其摸2张牌并展示，其中每有1张非基本牌，你令其恢复1点体力，以此法恢复的过量体力效果改为其摸等量的牌。',
						jlsg_xuanxin_info:'每回合限3次，当1名角色受到伤害后，你可以从弃牌堆获得1张红桃牌，然后可以交给其1张牌。',
						jlsg_lvezhen_info:'当你使用【杀】指定目标后，你可以将牌堆顶的3张牌置入弃牌堆，其中每有1张非基本牌，你弃置目标角色1张牌。',
						jlsg_youlong_info:'出牌阶段，若弃牌堆的牌数多于牌堆，你可以将黑色手牌当【顺手牵羊】使用。',
						jlsg_danqing_info:'出牌阶段限1次，你若已受伤可以选择1项：1.令1名其他角色摸X张牌，你将失去1点体力；2.令1名其他角色弃X张牌，你将恢复1点体力（X为你的当前体力）。',
						jlsg_zhonghun_info:'限定技，当你死亡时，你可以令1名其他角色获得你当前的所有技能。',			
						jlsg_jinji_info:"标记，当你拥有此标记时，你不可成为［奇门·改］的目标，称为［禁］。",
						jlsg_dunjia_info:"限定技，你的回合开始时，可以选择1名无［禁］的其他角色，该角色获得技能［奇门·改］。",
						jlsg_qimen_1_info:"变化技，锁定技，体力变动时，你随机获得未加入本局游戏的1个常规魏国武将的技能（限定技、变化技、主公技和觉醒技除外〖每局游戏最多发动1次，发动1次之后，你失去此技能获得技能［遁甲］直到游戏结束〗）。",
						jlsg_qimen_info:'变化技，锁定技，游戏开始时/体力变动时，你随机获得未加入本局游戏的1个常规魏国武将的技能（限定技、变化技、主公技和觉醒技除外〖每局游戏最多发动4次，发动4次之后，你失去此技能获得技能［遁甲］直到游戏结束〗）；遁甲：限定技，你的回合开始时，可以选择1名无［禁］的其他角色，该角色获得技能［奇门·改］；奇门·改：将3次数改成1次数，删除游戏开始时。',
                ...
                if(lib.device||lib.node){
                    for(var i in jlsg_soul.character){jlsg_soul.character[i][4].push('ext:民间极略/'+i+'.jpg');}
				}else{
					for(var i in jlsg_soul.character){jlsg_soul.character[i][4].push('db:extension-民间极略:'+i+'.jpg');}
				}
					return jlsg_soul;
				});
				lib.config.all.characters.push('jlsg_soul');
		...
		var sgs={
			...
			sort:{
				...
				defense:function(a,b){
					return sgs.getDefenseSha(a)<sgs.getDefenseSha(b);
				},
				threat:function(a,b){
					var d1=a.num('h');
					for(var i=0;i<game.players.length;i++){
						if(a.canUse('sha',game.players[i])&&a!=game.players[i]){
							d1=d1+10/(sgs.getDefense(game.players[i]))
						}
					}
					var d2=b.num('h');
					for(var i=0;i<game.players.length;i++){
						if(b.canUse('sha',game.players[i])&&b!=game.players[i]){
							d2=d2+10/(sgs.getDefense(game.players[i]))
						}
					}
					return d1>d2;
				}
			...
			needKongcheng:function(player,keep){
				if(keep){
					return sgs.isKongcheng(player)&&(player.hasSkill('kongcheng')||(player.hasSkill('zhiji')&&!player.storage.zhiji));
				}
				if(!sgs.hasLoseHandcardEffective(player)&&!sgs.isKongcheng(player)) return true;
				if(player.hasSkill('zhiji')&&!player.storage.zhiji) return true;
				return player.hasSkills(sgs.ai.skill.need_kongcheng);
			},
			...
			hasJiuEffect:function(player){
				...
				if(player.hasSkill('chouhai')&&sgs.isKongcheng(player)) return true;
			...
			loseCardEffect:function(player){	
				if(sgs.needKongcheng(player)) return 3;
				if(sgs.getLeastHandcardNum(player)>0) return 1;
				return -player.countCards('h');
			},
			gainCardEffect:function(player){
				if(sgs.needKongcheng(target,true)) return -1;
				if(sgs.getOverflow(player)) return 0;
				return 3;
			},
			getLeastHandcardNum:function(player){
				...
				var jwfy=sgs.findPlayerBySkillName('shoucheng');
				if(least<1&&jwfy&&sgs.isFriend(player,jwfy)) least = 1;
				if(player.hasSkill('shangshi')&&least<Math.min(2,sgs.getLostHp(player))) least=Math.min(2,sgs.getLostHp(player));
				return least;
			},
			hasLoseHandcardEffective:function(player){
				return player.countCards('h')>sgs.getLeastHandcardNum(player);
			},
			isWeak:function(player){
				if(sgs.hasBuquEffect(player)) return false;
			...
			isGoodHp:function(player){
				if(player.hp>1||sgs.getCardsNum('tao',player)>=1||sgs.getCardsNum('jiu',player)>=1) return true;
				if(sgs.hasBuquEffect(player)) return true;
			...
			isScure:function(player){
				if(player.hp>sgs.getBestHp(player)) return true;
				if(sgs.countCanShaMe(player)<=0) return true;
				if(sgs.isGoodHp(player)) return true;
				return false;
			},
			...
			cardNeed:function(card,player){
				...
				var friends=sgs.getFriends(player).sort(sgs.sort.hp);
				if(!friends.length) return null;
				if(card.name=='tao'){
					friends.sort(sgs.sort.hp);
					if(friends[0].hp<2) return 10;
					if(player.hp<3||(sgs.getLostHp(player)>1&&!player.hasSkills('longhun|buqu|jlsg_longhun'))||player.hasSkills('kurou|benghuai')) return 14;
					return sgs.getUseValue(card,player);
				}
				var wuguotai=sgs.findPlayerBySkillName('buyi');
				if(wuguotai&&sgs.isFriend(player,wuguotai)&&get.type(card)!='basic'){
					if(player.hp<3||(sgs.getLostHp(player)>1&&!player.hasSkills('longhun|buqu|jlsg_longhun'))||player.hasSkills('kurou|benghuai')) return 13;
				}
				if(sgs.isWeak(player)&&card.name=='shan'&&sgs.getCardsNum('shan',player,player)<1) return 12;
				return 0;
			},
			...
			willSkipPhaseUse:function(player){
				var friend_wuxie=0;
				for(var i=0;i<game.players.length;i++){
					if(sgs.isFriend(player,game.players[i])) friend_wuxie=friend_wuxie+sgs.getCardsNum('wuxie',game.players[i],player);
					if(sgs.isEnemy(player,game.players[i])) friend_wuxie=friend_wuxie-sgs.getCardsNum('wuxie',game.players[i],player);
				}
			...
			isCard:function(name,card,player){
				...
				if(card.name!=name){
					...
					if(!owner||player!=owner){
						...
						place=sgs.getCardPlace(card);
					}
					if(sgs.getSkillViewCard(card,name,player,place)) return true;
			...
			getKnownCard:function(player,from,card_name,viewAs,flags){
				...
				for(var i=0;i<cards.length;i++){
					...
					if((!forbid&&player==from)){
						if((viewAs&&sgs.isCard(card_name,card,player))||card.name==card_name||get.suit(card)==card_name||get.color(card)==card_name){
							know++;
						}
					}
				}
			...
			getDefenseSha:function(player,attacker){
				...
				var defense=sgs.getCardsNum('shan',player,attacker);
				var knownShan=sgs.getKnownCard(player,attacker,'shan',true);
				...
				if(sgs.getCardsNum('shan',player,_status.event.player)>1){
				...
				if(player.hasSkill('jlsg_danshou')&&!sgs.isKongcheng(player)&&!sgs.isKongcheng(attacker)) defense+=0.5;
				
				var jlsgsk_zhuran=sgs.findPlayerBySkillName('jlsg_yonglie');
				if(jlsgsk_zhuran&&sgs.isGoodHp(jlsgsk_zhuran)){
					if(get.distance(jlsgsk_zhuran,player,'attack')<=1&&sgs.isFriend(player,jlsgsk_zhuran)) defense+=0.5;
				}
				var jlsgsr_zhangliao=sgs.findPlayerBySkillName('jlsg_yansha');
				if(jlsgsr_zhangliao&&jlsgsr_zhangliao.storage.jlsg_yansha2&&jlsgsr_zhangliao.storage.jlsg_yansha2.length){
					if(sgs.isFriend(player,jlsgsr_zhangliao)&&get.attitude(jlsgsr_zhangliao,attacker)<0&&attacker.num('he')) defense+=0.5;
				}
				
				if(player.hasZhuSkill('hujia')){
					var caocao=player;
					var list=game.filterPlayer(function(target){
						return sgs.isFriend(target,caocao)&&target.group=='wei'&&target!=caocao;
					});
					if(list.length>0){
						var hujiaShan=0;
						for(var i=0;i<list.length;i++){
							hujiaShan+=sgs.getCardsNum('shan',list[i],_status.event.player);
							if(sgs.hasBaguaEffect(list[i])) hujiaShan+=0.8;
						}
						defense+=hujiaShan;
					}
				}
				...
				if(attacker&&!attacker.hasSkill('jueqing')){
					if(player.hasSkillTag('maixie')&&sgs.isGoodHp(player)) defense++;
				...
				if(player.hp>sgs.getBestHp(player)) defense+=0.8;
				if(player.hp<=2) defense-=0.4;
				if(player.hasSkill('tianxiang')) defense+=player.countCards('h')*0.5;
				
				if(player.countCards('e','tengjia')&&sgs.hasZhuqueEffect(attacker)&&!attacker.hasSkill('unequip')) defense-=0.6;
				if(player.isZhu){
					defense-=0.4;
					if(sgs.isZhuInDanger()) defense-=0.7;
				}
				...
				if(!sgs.hasBaguaEffect(player)){
					...
					if(player.hasSkill('guzheng')&&!sgs.getCardsNum('shan',player,attacker)) defense-=2.5;
			...
			getDefense:function(player){
				...
				if(!current_player) return sgs.getValue(player);
				
				var defense=sgs.getValue(player);
				...
				var maixie=sgs.ai.skill.maixie_skill.split("|");
				for(var i=0;i<maixie.length;i++){
					if(player.hasSkill(maixie[i])&&sgs.isGoodHp(player)) defense++;
				}
				...
				if(player.hasZhuSkill('shichou')){
					var current=sgs.findPlayerBySkillName('shichou_dying');
				...
				if(player.hasSkills(sgs.ai.skill.bad_skills)) defense--;
				
				if(player.isZhu){
					defense = defense - 0.4;
					if(sgs.isZhuInDanger()) defense = defense - 0.7;
				}
			...
			isZhuHealthy:function(){
				...
				return zhu_hp > 3||(zhu_hp>2&&sgs.getDefense(zhu)>3);
			},
			...
			getFriends:function(player){
				return game.filterPlayer(function(target){
					return sgs.isFriend(player,target);
				});
			},
			getFriendsNoself:function(player){
				return game.filterPlayer(function(target){
					return sgs.isFriend(player,target)&&player!=target;
				});
			},
			getEnemies:function(player){
				return game.filterPlayer(function(target){
					return sgs.isEnemy(player,target);
				});
			},
			...
			filterEnemy:function(player,func){
				var enemies=sgs.getEnemies(player);
			...
			countFriends:function(player){
				return game.countPlayer(function(target){
					return sgs.isFriend(player,target)&&target!=player;
				});	
			},
			...
			countCanShaMe:function(player){
				return game.countPlayer(function(target){
					return sgs.isEnemy(player,target)&&target.canUse('sha',player)&&get.effect(target,{name:'sha'},player)>0;
				});
			},
			getCanShaMe:function(player){
				return game.filterPlayer(function(target){
					return sgs.isEnemy(player,target)&&target.canUse('sha',player)&&get.effect(player,{name:'sha'},target)>0;
				});
			},
			...
			getCardsNum:function(class_name,player,from){
				...
				if(class_name=='sha'){
					...
					return (sgs.hasWushuangEffect(player)&&shanum*2)||shanum;
				}
...
config:{
	intro:{
		name:"<div><li>极略全部武将·联机版<li>可在联机武将设置里设为联机禁用<li>我将自己的SK补全和可乐大神的极略三国进行整合，从而使极略武将集齐<li>修复了极略三国的部分技能与ai的bug，优化技能描述与名称，增加缺少的配音<li>极略三国作者可乐，特此强调。本扩展旨在推广联机，整合极略</div>",
		clear:true,
	},
	...
	forbidBtCharacter:{
		name:'禁用变态（弱鸡）武将',
		intro:"变态（弱鸡）武将总包括：<br>SK向朗,SK陆绩,SK卞夫人,SK于禁,SK潘凤,SK邓芝,SK张布,SK孙策,SK马良,SK黄月英,SK董卓,SK丁奉,SK朱然,SK曹冲,SK张鲁,SK李严,SK卢植,SK张宁,SK郭女王,SK何进,SK王异,SR陆逊,SR吕蒙,SR曹操,SR黄月英,SR甄姬，SK神黄月英,SK神司马徽（以上仅个人观点）<li>SC武将有用得不顺眼的自个禁[吃瓜]",
		init:false,
	},
	oldCharacterReplace:{
		name:'旧版替换',
